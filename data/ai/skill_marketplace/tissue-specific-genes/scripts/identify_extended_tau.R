...
identify_extended_tau <- function(expression_matrix, tau_results, z_threshold = 1.96,
                                  tau_threshold = 0.85, min_max_expression = 10) {
  ...
  # 执行筛选：Tau 与最大表达量均达到调用者指定阈值
  # 使用 dplyr::filter
  specific_candidates <- dplyr::filter(
    gene_stats,
    Tau >= tau_threshold,
    Max_Exp >= min_max_expression
  )
  
  if (nrow(specific_candidates) == 0) {
    warning("No genes passed the requested Tau and maximum-expression thresholds.")
    return(NULL)
  }
