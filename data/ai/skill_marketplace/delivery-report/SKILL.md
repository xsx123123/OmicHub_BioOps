...
description: 当交付任务满足 HTML 报告阈值（≥3 个分析阶段 / 图表产物 ≥5 个 / 结果文件 ≥10 个 / 用户明确要求网页报告）且用户已在 ask_user 弹窗中确认后，使用本技能在
  Studio 沙盒中用 Python + Plotly + Jinja2 生成单文件 final-report.html。常规交付或用户未确认时只输出
