...
| 项 | 关键文件 |
|---|---|
| 看门狗双层（Bridge 900s / OmicHub 600s） | `integrations/agentteams/bridge/omichub_agentteams_bridge/service.py:257-272`；`src/omichub/infrastructure/celery_app/tasks/agentteams.py:188-210` |
| 5 个会诊只读工具 | `tool_configs/tools_schema.yaml:127-220`；`src/omichub/application/services/agentteams_data_tool_service.py` |
| 硬规则门 | `src/omichub/application/services/agentteams_quality_gate_service.py` |
| retry 幂等升版 | bridge `service.py:1551-1650`；`app.py:271-280` |
| 通用化 Registry（三处硬编码已消除） | `src/omichub/application/services/agentteams_capability_registry.py` |
...
Case 创建 → Bridge 规划/分配 work item
  ...
  → workspace_execution 实际在【主站侧】执行：
      ...
      → 工作目录 /data/omichub/output/agentteams/{case_id}/{work_item_id}/
...
2. `.env.example` / `.env`：新增 `MINIO_ROOT_USER` / `MINIO_ROOT_PASSWORD` / `MINIO_AGENTTEAMS_BUCKET` / `OMICHUB_MINIO_BIND_HOST` / `OMICHUB_MINIO_PORT` / `OMICHUB_MINIO_CONSOLE_PORT`（.env 中密码已生成随机值）。
...
```yaml
  ...
  # web 经 data_net 访问；omichub_net 供跨栈（agentteams bridge/worker）经服务名 minio 直连
  minio:
    image: minio/minio:latest
    container_name: omichub-minio
    ...
    ports:
      - "${OMICHUB_MINIO_BIND_HOST:-127.0.0.1}:${OMICHUB_MINIO_PORT:-9000}:9000"
      - "${OMICHUB_MINIO_BIND_HOST:-127.0.0.1}:${OMICHUB_MINIO_CONSOLE_PORT:-9001}:9001"
    volumes:
      - ${OMICHUB_DATA_ROOT:-/data/omichub}/omichub_data/_minio:/data
    networks:
      - data_net
      - omichub_net
    healthcheck:
  ...
  minio-init:
    ...
    container_name: omichub-minio-init
    logging: *default-logging
    entrypoint: ["/bin/sh", "-c"]
    command:
      - >
        mc alias set omichub http://minio:9000 "$${MINIO_ROOT_USER}" "$${MINIO_ROOT_PASSWORD}"
        && mc mb --ignore-existing "omichub/$${MINIO_AGENTTEAMS_BUCKET}"
        && mc anonymous set none "omichub/$${MINIO_AGENTTEAMS_BUCKET}"
    environment:
...
**设计要点**（不要擅自改动）：
- 端口绑 `127.0.0.1`（与 redis/postgres 同款安全姿势）；容器间走 `data_net`/`omichub_net` 服务名直连，不经宿主端口。
- 数据 bind mount 到 `${OMICHUB_DATA_ROOT}/omichub_data/_minio`（与 `_redis` 同款迁移姿势）。
- `omichub_net` 是 external 跨栈网络，agentteams 栈的 bridge 加入后即可 `http://minio:9000` 访问。
- bucket 默认私有（`anonymous set none`）。

**同时修改** `deploy/agentteams/docker-compose.agentteams.yml`：
- `omichub-agentteams-bridge` 服务的 `networks` 列表追加外部网络 `omichub_net`（引用已存在的 external 网络，不新建）。
...
docker exec omichub-minio mc ready local   # 注意：compose 姿势固定 -p docker，根 .env 的 COMPOSE_PROJECT_NAME 会顶掉默认项目名
docker run --rm --network omichub_net minio/mc alias set t http://minio:9000 $MINIO_ROOT_USER $MINIO_ROOT_PASSWORD && ... ls t/agentteams-evidence
```

### 1.2 P0-2 主站 S3 客户端封装

**施工**：新增 `src/omichub/infrastructure/storage/minio_store.py`：

- 依赖：`minio` Python SDK（加入 `pyproject.toml`/`requirements`，重建 web 镜像或在 Dockerfile runtime 安装——注意 web 镜像无热重载，装依赖必须重建镜像）。
- 配置读 `Settings`（`src/omichub/core/config.py` 新增，**裸名无前缀**，参照 SKILLS_DIR 先例）：
  ```python
...
**验证**：容器内 python REPL 上传/下载/预签名闭环；`docker restart omichub-web` 后工具可发现。

### 1.3 P0-3 产物上传改造（上游写）

**施工**：`src/omichub/application/services/agent_consultation_service.py` 的 `_register_workspace_artifacts`（当前 154-307 行）：
...
**Bridge 侧**：`integrations/agentteams/bridge/omichub_agentteams_bridge/service.py:621-631` 的 `_workspace_artifact_refs`：ContextRef 的 `location` 填 s3_uri（本地路径放 `meta["local_path"]` 兜底）。

**验证**：跑一个 treeplot Case → MinIO 控制台可见 `{case_id}/{work_item_id}/tree.pdf`；`/files` 仍正常显示。

### 1.4 P0-4 下游读取工具（解除 20KB 限制）

**施工**：`tool_configs/tools_schema.yaml` 新增只读工具 + `src/omichub/application/services/agentteams_data_tool_service.py` 实现：
...
**注册**：加入 `data/ai/tools/agentteams_case.yaml` 的 `builtin_tools`，使所有挂 agentteams_case tool_pack 的 agent 可用。改 builtin 预设后 **`docker restart omichub-web`**。
...
**施工**：
1. Bridge `models.py` 的 `ContextRef.kind` 枚举增加 `"s3"`（检查 `integrations/agentteams/gateway/omichub_agent_gateway/models.py` 同步）。
...
3. 本地 `/data/omichub/output/agentteams/{case_id}/` 执行工作目录同期清理（file_records 已复制走的不影响）。
...
**现状**：`POST /api/v1/agent-teams/cases` 端点存在（`src/omichub/api/v1/agentteams.py:238-265`），但前端建单仍经 LLM 工具调用（`create_agentteams_case`，`tools_schema.yaml:97` `requires_confirm: true`），不是"信息足够时直接弹确认卡"的零 LLM 路径。会诊卡"基于此创建协作 Case"按钮 UI 缺失（API 已支持 `origin_consultation_id`，`frontend/src/api/agentTeams.ts:18,59,81`）。
...
**施工**：
1. 后端：`src/omichub/api/v1/admin/` 下新增 `GET /admin/agentteams/consultation-telemetry?days=7`，AdminRequired（仅角色，非 TOTP——参照平台配置条件 TOTP 先例，只读指标免 TOTP）。
...
**施工**：改从 capability snapshot 读取 `workspace_capable_targets`（OmicHub 侧 `AgentTeamsCapabilityRegistry` 新增方法：凡 agent YAML 的 `capability_scope` 含 workspace 类能力或 tool_packs 含 `workspace` 的 active agent 即入选）。Bridge 启动拉取 + 定时刷新（沿用 capabilities 拉取链路）。
...
**施工**：Bridge 定时（60s）从 OmicHub `/capabilities` 拉 allowed_flow_ids，与 env 白名单取并集（env 为空则全信 Registry）；新增 flow YAML 后最晚 60s 可建单，无需重启。
...
### 5.5 不回归（每次施工后必跑）
⬜ Bridge/Gateway/Worker 契约测试 + OmicHub 单测回归。  
...
- **compose 姿势**：主栈必须 `cd deploy/docker && docker compose --env-file ../../.env -p docker up -d`（根 .env 的 COMPOSE_PROJECT_NAME=omichub 会顶掉项目名 docker）。
- **改后端代码 → `docker restart omichub-web`**（uvicorn --workers 2 无 reload）；改 Celery task → `docker restart omichub-worker`；builtin 工具/预设变更 → restart omichub-web。
- **装 Python 依赖 → 重建 web 镜像**（restart 不够）。
- file_records 是 `/files` 可见性的权威来源，任何产物链路改造不得绕过登记。
- 业务智能只写 OmicHub 侧；Bridge/Gateway/Worker 只做编排、搬运与审计。
...
| .env | OMICHUB_MINIO_BIND_HOST / PORT / CONSOLE_PORT | 127.0.0.1 / 9000 / 9001 |
