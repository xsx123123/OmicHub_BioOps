...
你是 OmicHub 仓库（/home/zj/zj_code_libarary/OmicHub）的施工工程师。本次任务：实施 **AgentTeams v2.2 专项**——MinIO agent 间共享存储 + v2.1 遗留收尾 + 并行/通用化增强。
...
先 `git diff deploy/docker/docker-compose.yml .env.example` 审查，与规格 §1.1 对照；可微调实现细节，但**网络（data_net/omichub_net）、端口绑定（127.0.0.1）、bucket 名（agentteams-evidence）、bind mount 路径（_minio）不得改**。
...
1. **compose 启动姿势**：主栈必须 `cd deploy/docker && docker compose --env-file ../../.env -p docker up -d <service>`。根 `.env` 的 `COMPOSE_PROJECT_NAME=omichub` 会顶掉默认项目名 `docker`，漏了 `-p docker` 会起出一套平行栈。
2. **无热重载**：改后端 Python 代码 → `docker restart omichub-web`；改 Celery task → `docker restart omichub-worker`；改 builtin 工具/工具预设 → restart omichub-web。`docker exec` 看到的是磁盘不是进程内存，改完不重启等于没改。
...
5. **业务智能只写 OmicHub 侧**（`src/omichub/`）；Bridge/Gateway/Worker（`integrations/agentteams/`）只做编排、搬运与审计。LLM prompt、阈值判断、业务规则禁止进 Bridge。
...
- P0-1：`minio-init` 退出码 0；`docker exec omichub-minio mc ready local` 通过；从 omichub_net 网络内能 `mc ls` 到 `agentteams-evidence` bucket 且为私有。
...
- P2/P3：对应规格的验收用例。
- 每项完成后：Bridge/Gateway/Worker 契约测试（`integrations/agentteams/bridge/tests/`、`gateway/tests/`）+ OmicHub 相关单测回归。
