...
你是 OmicHub 仓库（/home/zj/zj_code_libarary/OmicHub）的施工工程师。本次任务：实施 **Overdrive 超频模式 v1 专项——Manager 真规划改造**，让 Manager LLM 真正按任务动态分工，权威领域契约从"整体替换"降级为"校验护栏"。

### 第一步：读文档（先读后动，禁止跳读）

1. **施工规格（权威）**：`data/ai/update/Overdrive_update_v1.md`——所有施工项、契约、验收标准以此为准。
2. **读懂现状代码再改**：
   - `src/omichub/application/services/chat_service.py:592-691`（`_default_overdrive_assignments`）
   - `src/omichub/application/services/chat_service.py:281-308`（`OVERDRIVE_MANAGER_PROMPT`）
   - `src/omichub/application/services/chat_service.py:3100-3202`（规划主流程，覆盖点在 3168-3183）
   - `src/omichub/application/services/domain_registry.py`（领域包求值器）
   - `src/omichub/domain/domains/schema.py` + `data/ai/domains/phylo.yaml`（权威规则定义）
...
1. **无热重载**：改任何后端 Python 代码（chat_service / schema / config / prompt）→ `docker restart omichub-web`。`docker exec` 看到的是磁盘不是进程内存，改完不重启等于没改。
2. **compose 姿势**：live 栈 project 名是 `omichub`；需要重建容器时 `cd deploy/docker && docker compose --env-file ../../.env -p omichub up -d web`。改 `.env` 后 `docker restart` 无效（env 烘焙于容器创建时），必须 compose up 重建。
