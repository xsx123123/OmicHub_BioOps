...
晋升到 `data/ai/skill_marketplace/` 必须由管理员人工确认。
...
开始生成前必须确认以下信息；如果用户提供不足，用 `ask_user` 提问：
...
外部依赖时按 OSDP §2.3/§6.8 做 A/B/C/D 四级可达性标注，禁止编造安装命令或隐藏网络依赖。

## 方法论与专业决策

- 技能包必须是**自包含**的：脚本实体进 `scripts/`，参考手册进 `references/`，
  禁止写源仓库路径或宿主机绝对路径；
- 优先做**知识型/咨询型**技能；若涉及可执行脚本，必须补 `references/environment.md`
  和 `scripts/check_env.sh`；
...
- 禁止覆盖已有候选目录，除非用户明确说"替换"；
- 禁止在技能包内写入 API Key、内网地址、数据库连接串；
- 禁止声称"已挂载"或"已发布"，产物状态永远是 `candidate`。
...
3. **实体进包**：脚本必须复制进 `scripts/`，禁止写"详见源仓库"。
4. **路径合法**：只允许 `/workspace/.skills/<skill_id>/scripts/...`、`references/...`、`ref/...` 三种运行时形态。
5. **依赖可达**：C/D 级依赖必须如实标注，禁止写白名单外域名的安装命令。
