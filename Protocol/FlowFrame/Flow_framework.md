# Flow Framework — 多组学通用流程架构规范 v2.1
...
> **v2.1 修订说明（2026-07-26）**：
> 1. 新增 **§17 双模运行规范**：OmicHub 平台编排与 standalone 独立运行等价支持，平台依赖一律"可注入、可缺失、失败不阻塞"；
> 2. 与 ARDP（`Protocol/分析流程结果交付协议_v1.md`）对接：新增 §6.6 终态交付与失败兜底条款，§5.4 / §6.5 报告产物锚定 `{data_deliver}/report_data/`（修正原相对路径缺陷），`delivery_outputs()` 登记 `result_manifest.json` 目标；
> 3. §13.5 监控推送由"生产必须"改为"平台模式下必须、standalone 可选"，并新增推送失败不得中断分析的**非阻塞条款**；
> 4. §3.5 明确平台注入键（`platform` / `monitor_conf`）不得列入 schema `required`，保证独立运行的配置校验可通过。
...
# 失败兜底钩子：写出失败 manifest（见 §6.6，对接 ARDP 终态交付）
# onerror: ...
```
...
# ─── 平台集成（OmicHub 分析中心注入，standalone 运行不写）─
# platform:                        # ARDP §4.2 平台上下文块，流程原样回显，不解释
#   task_id: <uuid>
#   user_id: <uuid>
#   flow_id: rna_seq
# monitor_conf: /path/to/monitor_config.yaml   # §13.5 监控推送配置（可选）
```
...
> **平台集成键双模规则**：`platform` / `monitor_conf` 仅在 OmicHub 平台编排模式下由平台注入，standalone 模式缺失；流程代码一律 `config.get(...)` 读取、缺失走 fallback 分支，schema 不得将其列入 `required`（§3.5、§17.3）。
...
- **平台集成键不得必填**：`platform`、`monitor_conf` 等仅由 OmicHub 平台编排注入的键禁止列入 schema `required`；不含这些键的 standalone 配置必须能通过 schema 校验（§17.3-1）。`platform` 键声明为 `type: object` 透传即可，schema 不校验其内部字段。
...
def delivery_outputs(config) -> list:
    ...
    result_manifest.json 是面向 OmicHub 平台的终态交付清单（契约见
    ARDP：Protocol/分析流程结果交付协议_v1.md）。生成它的 rule 必须排在
    所有其他交付/报告产物之后（§6.6）；standalone 运行同样产出（platform
    块为空），保证产物可事后无损导入平台。
    """
    ...
    return [
        ...
        f"{deliver_dir}/result_manifest.json",
    ]

def report_outputs(config) -> list:
    """报告模块固定输出（最小实现）。

    必须锚定 data_deliver 下 report_data/（与交付层、ARDP 平台侧发现
    约定一致）。禁止返回相对路径：workdir: 重定向后相对路径落在
    workflow 目录，交付区与平台扫描均找不到该文件。
    """
    return [f"{config['data_deliver']}/report_data/project_summary.json"]
```
...
rule generate_project_summary:
    output: f"{config['data_deliver']}/report_data/project_summary.json"  # 锚定交付目录（§5.4）
...
- 报告降级为仅 `project_summary.json` 时（集群 executor 无容器路径），交付层仍须写出 `result_manifest.json`，`run.status` 置 `completed_no_report`（ARDP §4.3），平台报告中心据此隐藏预览按钮但保留文件下载。

### 6.6 终态交付与失败兜底（对接 ARDP，强制）

OmicHub 平台侧「结果报告中心」的注册完全依赖终态交付清单 `result_manifest.json`——字段契约、传输通道与平台消费规则见 `Protocol/分析流程结果交付协议_v1.md`（ARDP）。Flow 家族的实现必须满足：

1. **产物与 placement**：`{data_deliver}/result_manifest.json` 是交付层固定目标（§5.4 `delivery_outputs`）；生成规则放在 `N-1.deliver.smk`，`report: true` 时**必须**以 input 函数声明 HTML 报告与 `project_summary.json` 为依赖，保证它是整个运行的"最后一笔"（ARDP §3.2 原子写出、§3.3 写出时机）；`report: false` 时照常产出，`run.status=completed_no_report`。
2. **终态语义**：manifest 只表达终态（`completed` / `completed_no_report` / `failed` / `aborted`）。运行中进度走 §13.5 监控事件通道；禁止中途写出中间态 manifest，监控事件也不得替代 manifest（ARDP §6.1 两通道边界）。
3. **失败兜底**：snakefile **必须**有 `onerror` 钩子，失败时写出可写部分（`status=failed` + `error` 块：stage / rule / message / log_path）；钩子内部绝不允许再抛异常，写完后 `raise` 保持 Snakemake 失败语义。
4. **身份透传**：manifest 的 `platform` 块原样回显平台注入的 `config.platform`（ARDP §4.2）；standalone 运行时该块为空对象或缺失，合法。
5. **双模不阻塞**：manifest 写出失败（磁盘满/权限）只告警、不掩盖分析本身的完成状态；manifest 规则内禁止任何网络行为——推送不是流程的职责（§17.3）。
6. **字段对齐**：比较组名遵循 §8.5（`Treat_vs_Control`），`modules` 键与本流程 `MODULE_COLLECTORS` 注册表键一致（§5.1），`run.pipeline.version` 取自 `pipeline_version` 单一来源（§3.3）——三者直接流入 manifest（ARDP §4.6 / §4.9 / §4.3）。
...
对比名统一为 `Treat_vs_Control`（业界惯例，效应方向 = Treat 相对 Control）。RNAFlow 的 `id_convert.py` 曾生成 `Control_vs_Treat`（已验证），属于必须修正或显著标注方向含义的对齐项。对比名会原样流入结果交付清单的 `comparisons[].name`（§6.6、ARDP §4.6），平台不解析方向性，方向语义由本约定唯一负责。
...
- [ ] 报告规则至少生成 `{data_deliver}/report_data/project_summary.json`（锚定交付目录，§5.4），且 deliver/report 目标在 `rule all` 中
- [ ] `delivery_outputs()` 含 `result_manifest.json` 目标；`report: false` 时 manifest 规则正常产出 `completed_no_report`（§6.6）
- [ ] snakefile 有 `onerror` 钩子并能写出失败 manifest（§6.6-3）
- [ ] schema 的 `required` 不含 `platform` / `monitor_conf`；standalone 配置（无集成键）dry-run 通过（§3.5、§17.3）
- [ ] 监控端点不可达时运行不中断（§13.5 非阻塞条款）
...
从 **v0.2.0** 开始，`snakemake_logger_plugin_rich_loguru` 插件新增对 **OmicHub** 平台的原生工作流监控推送能力，同时保留原有 Loki/Grafana 兼容能力。**OmicHub 平台编排模式下必须配置监控推送**（由平台注入，见 §17.2）；**standalone 运行可选**——不配置 `monitor_conf` 即不推送，流程行为不变（双模总则见 §17）。
...
| `omichub_task_id` | `SNAKEMAKE_OMICHUB_TASK_ID` | 任务 ID：平台编排模式下必须等于平台注入块的 `platform.task_id`（§17.3-4）；standalone 模式可用 `project_name` 兜底 |
...
> **非阻塞条款（强制）**：监控推送是尽力而为（best-effort）行为——端点不可达、令牌失效、队列满、重试耗尽，一律降级为本地日志 WARNING，**禁止**使任何 rule 失败或中断整个运行。分析计算链路与平台集成链路在故障层面完全解耦：没有网络的环境（离线 HPC、气隙集群）必须能无差别跑完全部分析。另注意：监控事件只是运行中进度信号，不构成结果交付——终态交付由 `result_manifest.json` 唯一承担（§6.6、ARDP §6.1），两者共用 `task_id`/`user_id`/`flow_id` 三元组但互不替代。
...
> **project.yaml 集成**：OmicHub 平台下发任务时，通过 `project.yaml`（即 analysisyaml）同时注入 `platform` 上下文块（ARDP §4.2）与 `monitor_conf` 路径（§17.2），监控三元组与 `platform` 块逐字一致：`omichub_task_id` = `platform.task_id`、`omichub_user_id` = `platform.user_id`、`omichub_flow_id` = `platform.flow_id`（取值 `rna_seq`、`atac_seq`、`scrna_seq`、`btcr_seq`、`wgs`）。平台模式下 `omichub_task_id` 不得使用 `project_name`——`project_name` 只是业务展示名，用它做任务键会导致监控事件与终态 manifest 无法关联。
...
| 三家 | `report_outputs()` 相对路径 `project_summary.json` 改锚定 `{data_deliver}/report_data/`（本规范 v2.1 已修正条款；仓库代码随通用层同步） | §5.4 / §6.5 |
| 三家 | `delivery_outputs()` 登记 `result_manifest.json` 目标 + snakefile `onerror` 失败 manifest（对接 ARDP） | §5.4 / §6.6 |
| 三家 | 监控推送改平台模式必须、standalone 可选，落地推送失败非阻塞降级 | §13.5 / §17 |
...
5. `--logger rich-loguru` + `monitor_conf=monitor_config.yaml` 可推送事件到 OmicHub；**不配置** `monitor_conf` 时运行行为不变（§17）
6. §12.3 检查清单全部勾选
7. 至少一个真实小数据集端到端运行成功
8. **双模 dry-run**：standalone 配置（不含 `platform` / `monitor_conf`）与平台注入式配置（含两者）各 dry-run 通过一次（§17.3-5）

---

## 17. 双模运行：OmicHub 平台编排与 standalone 独立执行

所有 Flow 家族流程必须**等价支持**两种运行模式，且流程代码本身不对模式做分支（仅回显注入值）。这是强制架构约束：Flow 家族既是 OmicHub 分析中心的执行底座，也是可独立交付给课题组自行部署的生信流程。

### 17.1 两种模式的定义

| | 平台编排模式 | standalone 模式 |
|---|-------------|----------------|
| 触发方 | OmicHub 分析中心（Web / AI 助手提交） | 用户直接执行 `snakemake` 命令行 |
| analysisyaml | 平台 `generic_builder` 生成 | 用户自行编写（参考 `examples/analysisyaml.example.yaml`） |
| `platform` 块 | 平台注入（task_id / user_id / flow_id …，ARDP §4.2），流程原样回显 | 缺失（或空对象） |
| `monitor_conf` | 平台注入，进度推送至 `/api/v1/workflow-monitor/events`（§13.5） | 可选；不配置即不推送 |
| 结果去向 | 平台读取 `{data_deliver}/result_manifest.json` 自动注册「结果报告中心」（ARDP §6） | manifest 照常产出；可事后经 ARDP §6.3 HTTP 通道导入平台 |
| 身份三元组 | `platform.task_id` / `user_id` / `flow_id` 为唯一真源 | `project_name` / 本机用户 / 流程名兜底 |

### 17.2 注入契约（平台侧职责）

平台下发任务时经 analysisyaml 注入：

1. **`platform` 上下文块**：schema 与语义见 ARDP §4.2；流程 schema 必须容忍该键（§3.5）；
2. **`monitor_conf` 路径**：§13.5 监控配置，三元组与 `platform` 块逐字一致；
3. **标准键**：`workflow` / `data_deliver` / `sample_csv` / `reference_path` 等——与 standalone 完全同构，平台不引入任何专有路径布局。

### 17.3 双模不变量（强制）

1. **零硬依赖**：流程代码禁止直接索引 `config["platform"]`、禁止要求 `monitor_conf` 存在；所有平台集成读取一律 `config.get(...)` 且缺失有 fallback 分支。
2. **优雅降级**：平台设施的任何缺失或故障——注入缺失、端点不可达、令牌失效、推送超时——至多产生 WARNING 日志，**不得**使任何 rule 失败或整个运行中断（§13.5 非阻塞条款是其特例）。
3. **产物一致**：两种模式的交付产物完全相同（`data_deliver` 布局、`delivery_manifest.json`、`result_manifest.json`、报告），唯一差异是 manifest 的 `platform` 块有无取值；standalone 产物可随时无损导入平台（ARDP §6.3）。
4. **身份解析顺序**：监控/manifest 的身份字段按 `platform` 块 → `monitor_conf` 显式键 → `project_name` 兜底的顺序取值；平台模式下 `omichub_task_id` 禁止用 `project_name`。
5. **可测试**：流程 CI 必须包含两份配置的 dry-run——standalone 版（无集成键）与平台注入版（有集成键），任一失败视同 §12.2 回归失败。
