...
1. OmicHub 中提交给模型的提示词统一外置到 `data/`，代码只保存 key、变量和编排逻辑。
...
这里的“共享”必须严格理解为：两个执行器都可以引用 `omichub-analysis:plot-2026.07` 这个不可变镜像；Studio 仍创建会话级容器，工具箱仍创建任务级容器。
...
- `src/omichub/infrastructure/ai_provider/openai_compatible.py`
...
建议 tag 采用不可变版本，例如 `omichub-analysis:plot-2026.07`，不要在生产使用 `latest`。
...
python3 -m py_compile \
  src/omichub/infrastructure/config/prompt_loader.py \
  src/omichub/infrastructure/config/runtime_image_loader.py
