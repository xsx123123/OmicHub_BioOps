...
与 OmicHub.yaml 的 storage 段保持单一数据源，避免在此重复声明。
"""
...
class AutoScanConfig(BaseModel):
    ...
    enabled: bool = True
    scan_paths: list[str] = Field(default_factory=lambda: ["/data/omichub/users/{user_id}/bam/"])
...
def to_tracks_uri(abs_path: str) -> str:
    """把 /data/omichub/ref/rice/x.fasta 换算成 /tracks/ref/rice/x.fasta。
