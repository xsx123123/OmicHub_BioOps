...
class AssemblyDataFileDTO(OmicsHubBaseSchema):
...
class AssemblyDTO(OmicsHubBaseSchema):
...
class AssemblyListResponse(OmicsHubBaseSchema):
    assemblies: list[AssemblyDTO] = []


class PresetTrackDTO(OmicsHubBaseSchema):
    name: str
    file: str
    type: str
...
class PresetTracksResponse(OmicsHubBaseSchema):
    assembly_id: str
    tracks: list[PresetTrackDTO] = []


class ScannedFileDTO(OmicsHubBaseSchema):
    """用户目录扫描结果项。"""
...
class ScanResponse(OmicsHubBaseSchema):
    user_id: str
    scan_time: str
    total_files: int = 0
    indexed_count: int = 0
...
class UploadResultDTO(OmicsHubBaseSchema):
    """单个文件上传结果。"""
...
class BatchUploadItemDTO(OmicsHubBaseSchema):
    filename: str
...
class BatchUploadResponse(OmicsHubBaseSchema):
    total: int = 0
    success: int = 0
    failed: int = 0
...
class IndexStatusDTO(OmicsHubBaseSchema):
    """文件索引状态。"""
...
class IndexTaskSubmitDTO(OmicsHubBaseSchema):
    message: str
    task_id: str
    file: str
    status: str = "queued"
...
class IndexTaskStatusDTO(OmicsHubBaseSchema):
    """Celery 索引任务状态。"""
...
class ConfigReloadResponse(OmicsHubBaseSchema):
    message: str
