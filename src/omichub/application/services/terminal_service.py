...
from omichub.application.schemas.terminal import (
...
from omichub.domain.terminal.services import TerminalDomainService
...
from omichub.infrastructure.database.repositories.terminal_repository import (
    SqlAlchemyTerminalSessionRepository,
)
from omichub.infrastructure.terminal import get_terminal_docker_manager
from omichub.infrastructure.terminal.docker_manager import TerminalDockerError
from omichub.tools.terminal.config import (
    ResourceConfig,
    TerminalImage,
    get_terminal_config,
...
class TerminalService:
    ...
    async def list_all_sessions(self, include_stats: bool = False) -> list[AdminTerminalSessionDTO]:
        ...
        for s in sessions:
            ...
            container_name = (
                f"omichub-term-{str(s.user_id)[:8]}-{s.session_id}" if s.session_id else None
            )
