...
from omichub.application.services.agent_handoff_service import HANDOFF_TOOL_NAME
...
from omichub.infrastructure.database.models.agent import (
    AgentTemplateModel,
...
from omichub.infrastructure.mcp.conda_meta_preset import (
...
_PRESET_IDS_BY_NAME: dict[str, uuid.UUID] = {
    "omichub-platform": OMICHUB_PLATFORM_SERVER_ID,
    OMICHUB_PIPELINES_SERVER_NAME: OMICHUB_PIPELINES_SERVER_ID,
    OMICHUB_TOOLS_SERVER_NAME: OMICHUB_TOOLS_SERVER_ID,
...
class AgentService:
        ...
        for cfg in yaml_configs:
            ...
            if existing is not None:
                ...
                if existing.is_builtin:
                    ...
                    # 核心 builtin 预设 omichub-tools（富集/绘图/记忆/交接/子Agent/Case 工具）
                    # 与 omichub-platform（工作区文件）始终保留，其余 server 一律以声明为准。
                    declared_mcp_ids = [
                        str(value)
    ...
    async def assemble_context(self, agent_id: str, user_id: str | None = None) -> AgentContext | None:
        ...
        runtime_model_id = agent.model_id
        ...
        if user_id is not None:
            ...
            if capability is not None:
                ...
                runtime_model_id = capability.model_id or agent.model_id
        ...
        omichub_selected = OMICHUB_TOOLS_SERVER_ID in mcp_uuids
        other_uuids = [u for u in mcp_uuids if u != OMICHUB_TOOLS_SERVER_ID]
        if other_uuids:
            ...
            for preset_id, preset_name in (
                (OMICHUB_PLATFORM_SERVER_ID, "omichub-platform"),
                (OMICHUB_PIPELINES_SERVER_ID, OMICHUB_PIPELINES_SERVER_NAME),
        ...
        if OMICHUB_PLATFORM_SERVER_ID in mcp_uuids and not any(
            server.id == OMICHUB_PLATFORM_SERVER_ID or server.name == "omichub-platform"
            for server in mcp_servers
        ):
            platform_preset = get_preset_by_name("omichub-platform")
            if platform_preset is not None:
                ...
                platform_tools = [
                    MCPToolRegistry(
                        ...
                        server_id=OMICHUB_PLATFORM_SERVER_ID,
                    )
                    for tool in preset_tools
                ]
                mcp_servers.append(
                    MCPServer(
                        id=OMICHUB_PLATFORM_SERVER_ID,
        ...
        # 2.5 自动注入 omichub-tools builtin preset（显式选中，或为空时保持默认）
        omichub_tools_server = self._build_omichub_tools_server()
        if omichub_tools_server and (omichub_selected or not mcp_uuids):
            mcp_servers.append(omichub_tools_server)
        ...
        if skills:
            from omichub.domain.skill.services import build_skill_tools
        ...
        if any(server.id == OMICHUB_PLATFORM_SERVER_ID for server in mcp_servers):
            system_prompt = (
                f"{system_prompt}\n\n{WORKSPACE_FILES_SYSTEM_PROMPT_SUFFIX}"
                if system_prompt
            ...
            catalog.append(
                {
                    ...
                    "not_suitable_for": ability.get("not_suitable_for", []),
                    "handoff_when": ability.get("handoff_when", []),
                    "preferred_inputs": ability.get("preferred_inputs", []),
                }
            )
        if not catalog:
            return ""
    ...
    @staticmethod
    def _build_omichub_tools_server() -> MCPServer | None:
        """构造虚拟的 omichub-tools builtin MCP Server。"""
        tools = schema_loader.list_tools()
        if not tools:
            return None
        return MCPServer(
            id=OMICHUB_TOOLS_SERVER_ID,
            name=OMICHUB_TOOLS_SERVER_NAME,
            description="OmicHub 生信工具箱 MCP",
            transport=Transport.BUILTIN,
            is_enabled=True,
            is_preset=True,
            status=ServerStatus.ONLINE,
            tools=[
                MCPToolRegistry(
                    ...
                    server_id=OMICHUB_TOOLS_SERVER_ID,
                )
                for t in tools
            ],
        )
