...
class SubmitBuildRequest(OmicsHubBaseSchema):
...
class GenerateCodeRequest(OmicsHubBaseSchema):
    """仅生成代码（不落库注册）"""
...
class MCPBuildDTO(OmicsHubBaseSchema):
    """MCP 构建记录"""
...
class ReviewRequest(OmicsHubBaseSchema):
    """审核决定"""
...
class SandboxTestRequest(OmicsHubBaseSchema):
    """沙箱内测试构建"""
...
class ToolInvokeRequest(OmicsHubBaseSchema):
    """调用实验 MCP 工具"""
...
class RenewRequest(OmicsHubBaseSchema):
    """续期实验 MCP"""

    hours: int | None = None


class ExperimentalServerDTO(OmicsHubBaseSchema):
...
class VersionDTO(OmicsHubBaseSchema):
    """MCP 版本快照"""
