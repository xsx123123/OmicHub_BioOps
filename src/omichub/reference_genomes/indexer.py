...
CLI 用法::
    ...
    python -m omichub.reference_genomes.indexer build --version tair10
    python -m omichub.reference_genomes.indexer build --version tair10 --only gff3
    python -m omichub.reference_genomes.indexer audit
...
from omichub.reference_genomes.sequence import build_fai
...
def _parse_gff3(conn: sqlite3.Connection, version: VersionConfig, _log: Any) -> None:
    """解析 GFF3：gene / mRNA / exon → genes + transcripts。"""
...
def _parse_gaf(conn: sqlite3.Connection, version: VersionConfig, _log: Any) -> None:
    ...
    count = 0
    with opener(fpath, "rt", encoding="utf-8") as f:  # type: ignore[call-overload]
        for line in f:
            if line.startswith("!") or not line.strip():
                continue
            cols = line.rstrip("\n\r").split("\t")
    ...
    if is_full:
        ...
        if not db_path.exists():
            raise FileNotFoundError(
                ...
                f"  python -m omichub.reference_genomes.indexer build --version {version_id}"
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="python -m omichub.reference_genomes.indexer",
