...
设计：
- ``OmicHubSpanProcessor`` 在 span 启动时（仍在业务上下文内）把当前 ContextVar 里的
  session_id / request_id 写为 span 属性（session.id / request.id），使每个 span 都可
  关联到会话与请求。
- ``JsonlSpanExporter`` 把完成的 span 序列化为 JSONL 写入 ``omichub.spans.json.log``
...
SPAN_LOG_NAME = "omichub.spans.json.log"
_KEEP_BACKUPS = 3
...
class OmicHubSpanProcessor(SpanProcessor):
