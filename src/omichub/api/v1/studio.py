...
from omichub.application.services.studio_approval_service import get_studio_approval_service
...
from omichub.infrastructure.studio.control_client import resolve_studio_control_token
from omichub.infrastructure.studio.manager import (
    StudioSandboxUnavailableError,
    studio_sandbox_manager,
)
from omichub.infrastructure.studio.paths import PathEscapeError, resolve_workspace_read_path
...
def _schedule_sandbox_prewarm(
    session_id: str, user_id: str, image: str | None
) -> None:
    ...
    async def _prewarm() -> None:
        try:
            await studio_sandbox_manager.ensure_running(
                session_id, image=image, user_id=user_id
            )
...
async def internal_studio_exec(
    ...
    control_token: Annotated[
        str | None, Header(alias="X-OmicHub-Studio-Control-Token")
    ] = None,
) -> StreamingResponse:
...
async def update_studio_permissions(
    ...
    await db.execute(
        ...
        {"patch": json.dumps({"mode": req.mode}), "sid": session_id},
    )
    await db.flush()
    await db.refresh(session)
...
async def approve_studio_tool(
    ...
    resolved = await service.resolve(
        approval_id, str(current_user_id), action, modified_args=req.modified_args
    )
