...
class HeroContent(CygnusXBaseSchema):
    """Hero 欢迎区文案"""
...
class QuickEntryContent(CygnusXBaseSchema):
    """快捷入口兜底文案。
...
class GuideStepContent(CygnusXBaseSchema):
    """使用引导步骤文案"""
...
class RegistrationContent(CygnusXBaseSchema):
    """注册策略提示文案（公开读取）。

    真正的注册开关由后台 site-settings（DB）控制并在 /auth/register 强制校验；
    这里只提供“关闭注册时告诉用户联系谁”的文案，由 data/CygnusX.yaml 驱动。
    """
...
class ActivationContent(CygnusXBaseSchema):
    """账户未激活提示文案（公开读取）。

    用户登录未激活账户时（auth_service 抛“账户未激活”），前端弹出友好提示，
    引导用户联系管理员激活。文案由 data/CygnusX.yaml 的 activation 段驱动，改文件即生效。
...
class EasterEggContent(CygnusXBaseSchema):
    """关于页彩蛋文案（公开读取）。

    在「关于」页点击蓝色星球触发的隐藏弹窗，文案由 data/CygnusX.yaml 的
    easter_egg 段驱动，改文件即生效。
    """

    message: str = "🎉 恭喜你发现了 CygnusX 的隐藏星际守护者！"
    button_text: str = "🐾 召唤星际猫咪"
    toast: str = "🎉 星际守护者已响应召唤！快看看屏幕上留下的足迹吧～ ✨"


class PlatformContent(CygnusXBaseSchema):
    """平台元信息（公开读取）。

    由 data/CygnusX.yaml 的 platform 段驱动，提供作者、联系方式、GitHub 等。
    """
...
class AIAssistantContent(CygnusXBaseSchema):
    """AI 助手页文案（公开读取）。

    由 data/CygnusX.yaml 的 ai_assistant 段驱动，改文件即生效。
    """

    input_hint: str = "内容由 AI 生成，请仔细甄别"


class SiteContentDTO(CygnusXBaseSchema):
    """首页文案（公开读取）"""
