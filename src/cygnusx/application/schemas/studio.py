...
class CreateStudioSessionRequest(BaseModel):
    ...
    镜像选择有意区分“Agent 默认画像”和“用户显式能力覆盖”：

    * ``sandbox_capabilities is None`` 表示用户没有提出镜像覆盖要求，后端必须
      沿用 Agent YAML 中声明的 ``studio.image`` / ``runtime_profile``；
    * 只有用户明确传入能力列表时，后端才允许按能力匹配镜像（例如明确要求
      ``browser`` 才切换到 browser-office）。

    不要把 ``sandbox_capabilities`` 改回带有 ``code/browser/document`` 的默认值。
    那会让所有 Agent 看起来都需要浏览器和文档能力，并覆盖 agent-viz 的
    ``analysis-plot``、单细胞 Agent 的 ``analysis-scrna`` 等专用镜像。
    如果要改变这个优先级或默认行为，必须先向用户说明影响范围并获得确认。
    """

    agent_id: str = Field(..., description="绑定的 Agent ID")
    title: str | None = Field(None, description="会话标题，缺省为「与 <Agent名> 的工作台」")
    project_id: str | None = Field(
        None, max_length=64, description="项目边界标识；缺省自动归入「默认项目」"
    )
    model_id: UUID | None = Field(None, description="模型配置 ID，缺省用 Agent 绑定模型")
    sandbox_capabilities: list[Literal["code", "browser", "document"]] | None = Field(
        None,
        description=(
            "可选的沙盒运行能力；未提供时沿用 Agent 声明的 runtime_profile/image，"
            "显式提供时才按能力匹配镜像"
        ),
    )
    runtime_profile: str | None = Field(
        None,
        description="显式指定运行时 profile（见 runtime_images.yaml）；优先于按能力匹配的镜像",
    )
...
class PromoteStudioSessionRequest(BaseModel):
    ...
    runtime_profile: str | None = Field(
        None,
        description="显式指定运行时 profile；优先于目标 Agent studio.image",
    )
...
class StudioApprovalApproveRequest(BaseModel):
    ...
    always=True 表示"本会话不再询问"：批准后 (session, tool) 记入会话
    permissions.always_allow，同会话同工具后续直接放行；切换权限模式时清空。
    """

    modified_args: dict[str, Any] | None = Field(None, description="编辑后的工具参数")
    always: bool = Field(False, description="本会话内对该工具不再逐次审批")
...
class StudioSessionDetailDTO(BaseModel):
    ...
    sandbox_capabilities: list[Literal["code", "browser", "document"]] = Field(
        default_factory=lambda: ["code"],
        description="会话实际分配的沙盒运行能力（sandbox_meta.sandbox_capabilities）",
    )
    ...
    env_restore: dict[str, Any] | None = Field(
        None,
        description="最近一次声明式环境还原结果（status/file/duration_ms/reason）；"
        "failed 时前端给一次性 warning 提示（WP2 任务 3）",
    )
...
class SharedStudioMessageDTO(BaseModel):
    ...
    content: str
    created_at: datetime
    # R4（WP3 任务 4）：执行历史摘要（tool_invocations/timeline/usage），
    # 经 WP0 200KB 截断护栏处理；无工具执行的消息为 None
