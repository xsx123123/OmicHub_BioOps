...
class GoalStartRequest(CygnusXBaseSchema):
...
class GoalControlRequest(CygnusXBaseSchema):
    reason: str = Field(default="", max_length=2000)


class GoalAnswerRequest(CygnusXBaseSchema):
    answer: str = Field(min_length=1, max_length=10000)


class GoalResponse(CygnusXBaseSchema):
    id: UUID
    session_id: str | None
    user_id: UUID
...
class GoalEventResponse(CygnusXBaseSchema):
    id: UUID
    goal_id: UUID
    sequence: int
    event_type: str
...
class GoalEventPage(CygnusXBaseSchema):
