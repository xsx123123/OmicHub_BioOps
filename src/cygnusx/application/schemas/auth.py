...
class TokenResponse(CygnusXBaseSchema):
    """Token 响应体"""
...
class LoginResponse(CygnusXBaseSchema):
    """登录响应（兼容 2FA 拦截）。
...
class TwoFactorCodeRequest(CygnusXBaseSchema):
...
class TwoFactorLoginRequest(CygnusXBaseSchema):
    """2FA 登录校验请求体。"""
...
class TwoFactorSetupResponse(CygnusXBaseSchema):
...
class TwoFactorStatusResponse(CygnusXBaseSchema):
    """当前用户 2FA 状态。"""
