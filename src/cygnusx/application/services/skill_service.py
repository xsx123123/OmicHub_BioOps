...
from cygnusx.domain.skill.services import SkillDomainService
...
from cygnusx.infrastructure.skills.skill_store import rewrite_skill_md
from cygnusx.infrastructure.skills.skillmd import ParsedSkill
...
class SkillService:
    ...
    async def delete_skill(self, skill_id: str, force: bool = False) -> bool:
        ...
        from cygnusx.infrastructure.skills.skill_store import remove_skill_folder
    ...
    def _to_dto(m: SkillModel) -> SkillDTO:
        # bio- 前缀技能统一归为 analysis（历史已入库数据在读取侧归一化，免批量迁移）
        category = m.category
        if category != "analysis" and str(m.skill_id or "").startswith("bio-"):
            category = "analysis"
        return SkillDTO(
            id=m.id,
            skill_id=m.skill_id,
