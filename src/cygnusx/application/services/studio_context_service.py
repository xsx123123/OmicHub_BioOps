...
from cygnusx.core.exceptions import BusinessError, CygnusXError, NotFoundError
...
from cygnusx.infrastructure.config.runtime_image_loader import resolve_studio_image
...
from cygnusx.infrastructure.studio.paths import PathEscapeError
...
async def create_session_from_report(
    ...
    # 报告中心是系统代建会话，不经过 /studio/sessions 的请求解析层；必须
    # 复用同一 Agent image/profile 解析规则，否则旧记录只有 runtime_profile
    # 时会在沙盒管理器中静默落到 analysis-core。
    ...
    dto = await service.create_session(
        ...
        # 从报告一键优化由系统代建会话，报告本身不一定落在项目内，走内部豁免
...
async def register_artifact_report(
    ...
    async def persist(anchor_db: AsyncSession) -> tuple[ReportModel, ReportFileModel]:
        ...
        try:
            ...
            # checksum 列写入登记时对 dest 落盘文件的 sha256 实测（流式分块），
            # 实测不到（远程后端等）留空，与归档文档的 md5/sha256 语义一致，不阻断登记。
            artifact_sha256 = sha256_stream(dest) if dest.is_file() else ""
            try:
                ...
                await registry.register(
                    ...
                    checksum=artifact_sha256 or None,
                )
    ...
    if not isinstance(db, AsyncSession):
        ...
        async with get_session_factory()() as anchor_db:
            result = await persist(anchor_db)
    # 平台级归档文档：补写 run README / environment.json 并更新项目 AGENTS.md。
    # 失败不阻断登记主流程（报告已提交，文档可后续补生成）。
    report, file_model = result
    try:
    ...
    except Exception as exc:  # noqa: BLE001 - 归档文档失败不阻断产物登记
        logger.error("Studio 产物归档文档生成失败（非阻塞）: {}", exc)
    return report, file_model
