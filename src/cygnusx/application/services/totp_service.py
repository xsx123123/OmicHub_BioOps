...
from cygnusx.core.exceptions import AuthenticationError, BusinessError, NotFoundError
...
from cygnusx.domain.user.value_objects import UserStatus
...
class MfaService:
    ...
    def build_otpauth_uri(username: str, secret: str) -> str:
        """拼接 otpauth://totp/CygnusX:{username}?secret=...&issuer=CygnusX。

        pyotp 在 issuer_name 给定时会自动产出 "{issuer}:{name}" 形式的 label，
        故 name 只传 username，避免出现 "CygnusX:CygnusX:alice" 双前缀。
        """
