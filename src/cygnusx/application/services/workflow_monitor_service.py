...
class WorkflowMonitorService:
    ...
    def _normalize_native_event(self, payload: dict[str, Any]) -> WorkflowEvent:
        task_id = str(
            payload.get("task_id")
            or payload.get("cygnusx_task_id")
        ...
        return WorkflowEvent(
            task_id=task_id,
            flow_id=payload.get("flow_id") or payload.get("cygnusx_flow_id"),
            user_id=payload.get("user_id") or payload.get("cygnusx_user_id"),
    ...
    def _events_from_loki_payload(self, payload: dict[str, Any]) -> list[WorkflowEvent]:
        ...
        for stream in payload.get("streams") or []:
            ...
            for ts_ns, raw in stream.get("values") or []:
                ...
                task_id = str(
                    content.get("task_id")
                    or content.get("cygnusx_task_id")
                    or project_id
                    or ""
                )
                event = WorkflowEvent(
                    task_id=task_id,
                    flow_id=content.get("flow_id") or content.get("cygnusx_flow_id"),
                    user_id=content.get("user_id") or content.get("cygnusx_user_id"),
