...
任务命名遵循 storage.py 约定：cygnusx.tools.jbrowse.tasks.<name>，
...
@shared_task(
    name="cygnusx.tools.jbrowse.tasks.index_file",
    bind=True,
    max_retries=2,
...
@shared_task(name="cygnusx.tools.jbrowse.tasks.scan_and_index")
...
async def _scan_and_index(user_id: str) -> dict:
    from cygnusx.tools.jbrowse.config import config_manager
    from cygnusx.tools.jbrowse.service import jbrowse_service
