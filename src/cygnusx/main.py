...
from cygnusx.api.v1.router import api_router
...
from cygnusx.infrastructure.celery_app.celery import celery_app  # noqa: F401
from cygnusx.middleware.audit import AuditMiddleware
from cygnusx.middleware.auth import AuthMiddleware
from cygnusx.middleware.cookie import CookieRequiredMiddleware
from cygnusx.middleware.module_gate import ModuleGateMiddleware
from cygnusx.middleware.rate_limit import RateLimitMiddleware
from cygnusx.middleware.trace_context import TraceContextMiddleware
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    from cygnusx.core.telemetry import setup_telemetry
    ...
    from cygnusx.infrastructure.config.module_registry import init_module_registry

    registry = init_module_registry()
    logger.info(f"模块注册表已加载，共 {len(registry.modules)} 个模块")

    from cygnusx.infrastructure.database.session import init_db
    ...
    async def _warmup_sandbox() -> None:
        try:
            ...
            try:
                ...
                from cygnusx.infrastructure.database.models.sandbox import (
                    SandboxSessionModel,
                )
                from cygnusx.infrastructure.database.session import get_session_factory
    ...
    async def _sync_ai_providers() -> None:
        ...
        try:
            from cygnusx.application.services.ai_provider_yaml_loader import (
    ...
    async def _init_mcp_presets() -> None:
        # 初始化/同步内置 MCP 预设（cygnusx-platform / cygnusx-tools / cygnusx-pipelines），
    ...
    async def _init_default_knowledge() -> None:
        """首次部署时从 docs/knowledge/meta.yaml 初始化实验室知识库。"""
        try:
            from cygnusx.application.services.knowledge_bootstrap_service import (
            ...
            if factory is not None:
                async with factory() as session:
                    updated, created, skipped = await ensure_default_knowledge(session)
                    await session.commit()
                    if updated or created:
                        logger.info(
                            "默认知识库已从 meta.yaml 同步：更新 {} 篇，新建 {} 篇，跳过 {} 篇",
                            updated,
                            created,
                            skipped,
                        )
        except Exception as e:
            logger.warning(f"默认知识库初始化失败，降级运行: {e}")
    ...
    async def _seed_welcome() -> None:
        try:
            from cygnusx.application.services.welcome_service import WelcomeService
            from cygnusx.infrastructure.database.session import get_session_factory
    ...
    async def _auto_reindex_knowledge() -> None:
        ...
        try:
            ...
            from cygnusx.application.services.knowledge_index_service import (
    ...
    from cygnusx.infrastructure.database.session import close_db
...
def create_app() -> FastAPI:
    ...
    from cygnusx.core.telemetry import instrument_app, mount_metrics
    ...
    async def cygnusx_error_handler(request: Request, exc: CygnusXError) -> JSONResponse:
