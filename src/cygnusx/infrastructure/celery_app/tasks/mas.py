...
from cygnusx.infrastructure.mas.redis_streams import RedisStreamConsumer


@shared_task(name="cygnusx.infrastructure.celery_app.tasks.mas.publish_outbox")
...
@shared_task(name="cygnusx.infrastructure.celery_app.tasks.mas.consume_events")
def consume_events() -> dict[str, int]:
    return asyncio.run(_consume_events())


async def _consume_events() -> dict[str, int]:
    from cygnusx.application.services.mas_event_consumer_service import MASEventConsumerService
    from cygnusx.infrastructure.database.session import get_session_factory
...
@shared_task(name="cygnusx.infrastructure.celery_app.tasks.mas.run_fake_node")
...
@shared_task(name="cygnusx.infrastructure.celery_app.tasks.mas.rnaflow.run_node")
...
async def _run_rnaflow_node(run_id: UUID, node_key: str) -> dict[str, str]:
    ...
    from cygnusx.infrastructure.database.repositories.mas_repository import MASRepository
    from cygnusx.infrastructure.database.session import get_session_factory
    from cygnusx.infrastructure.mas.apptainer import build_rnaflow_command
    from cygnusx.infrastructure.mas.qc_metrics import QCMetricsError, extract_median_mapping_rate
...
async def _save_validated_file_artifact(
    ...
    from cygnusx.domain.mas.models import ArtifactKind, MASArtifact
...
async def _record_node_failure(
    ...
    from cygnusx.domain.mas.models import A2AEvent, A2AEventType, AgentRecipient, ArtifactPointer
...
@shared_task(name="cygnusx.infrastructure.celery_app.tasks.mas.volcano.run_node")
...
async def _run_volcano_node(run_id: UUID, node_key: str) -> dict[str, str]:
    ...
    from cygnusx.infrastructure.database.repositories.mas_repository import MASRepository
    from cygnusx.infrastructure.database.session import get_session_factory
    from cygnusx.infrastructure.mas.volcano import VolcanoPlotError, render_deg_volcano
...
@shared_task(name="cygnusx.infrastructure.celery_app.tasks.mas.quality_gate.run_node")
...
async def _run_quality_gate_node(run_id: UUID, node_key: str) -> dict[str, str]:
    ...
    from cygnusx.domain.mas.quality_gate import QCOverrideRequest, apply_qc_override, evaluate_mapping_rate
    from cygnusx.domain.mas.workspace import WorkspaceLayout
    from cygnusx.infrastructure.database.repositories.mas_repository import MASRepository
    from cygnusx.infrastructure.database.session import get_session_factory
...
@shared_task(name="cygnusx.infrastructure.celery_app.tasks.mas.ebi_download.run_node")
...
async def _run_ebi_download_node(run_id: UUID, node_key: str) -> dict[str, str]:
    ...
    from cygnusx.infrastructure.database.repositories.mas_repository import MASRepository
    from cygnusx.infrastructure.database.session import get_session_factory
    from cygnusx.infrastructure.mas.ebi import EBIDownloadError, build_ebi_download_command, write_download_manifest
...
@shared_task(name="cygnusx.infrastructure.celery_app.tasks.mas.scanpy_qc.run_node")
...
async def _run_scanpy_qc_node(run_id: UUID, node_key: str) -> dict[str, str]:
    ...
    from cygnusx.application.services.artifact_service import ArtifactService
    from cygnusx.core.config import get_settings
    from cygnusx.domain.mas.models import A2AEvent, A2AEventType, AgentRecipient, AgentSender, ArtifactPointer, NodeState
    from cygnusx.domain.mas.workspace import WorkspaceLayout
    from cygnusx.infrastructure.database.repositories.mas_repository import MASRepository
    from cygnusx.infrastructure.database.session import get_session_factory
