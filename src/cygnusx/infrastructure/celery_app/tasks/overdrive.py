...
def _refresh_instance_status_line(instance: dict[str, Any], phase: str) -> None:
    ...
    from cygnusx.application.services.agentteams_capability_registry import persona_status_line
...
def _overdrive_flow_version(run: Any) -> str:
    """归档/交付文档中的流程版本：冻结计划版本 + hash 前缀。"""
    plan = run.plan or {}
    version = int(plan.get("version") or 0)
    digest = str(plan.get("hash") or "")
    return f"v{version}" + (f" (hash {digest[:8]})" if digest else "")


@shared_task(name="cygnusx.infrastructure.celery_app.tasks.overdrive.advance_run")
...
def _overdrive_workspace_context(run: Any, task: dict[str, Any]) -> str:
    ...
    from cygnusx.application.services.overdrive_run_service import relative_overdrive_run_root
...
@shared_task(name="cygnusx.infrastructure.celery_app.tasks.overdrive.replan_run")
...
async def _replan_run(run_id: str) -> dict[str, object]:
    ...
    try:
        async with get_session_factory()() as db:
            ...
            if run is None:
                return {"status": "missing", "run_id": run_id}
            manager_ctx = await AgentContextBuilder(db).assemble(
                str(run.lead_planner_agent_id or "agent-general"),
                user_id,
                mode="overdrive",
            )
    ...
    async with get_session_factory()() as db:
        ...
        if recruited or resumed or reapproved or repaired or manager_jobs:
            ...
            environment = await collect_session_environment(
                ...
                flow_id="overdrive",
                flow_name="超频协作",
                flow_version=_overdrive_flow_version(run),
            )
            readme = await DeliveryAssembler().write_readme(run, delivery, environment=environment)
            run.artifact_index = [
                *run.artifact_index,
                {
            ...
            # 项目归档：会话绑定 project_id 时把交付物复制进项目 runs 目录并生成
            # AGENTS.md/README/environment.json；归档失败不阻断 run 完成。
            try:
                await archive_overdrive_delivery(
                    db,
                    run,
                    delivery_paths=[
                        delivery["path"],
                        str(delivery.get("archive_path") or ""),
                        readme["path"],
                    ],
                    environment=environment,
                )
            except Exception as exc:  # noqa: BLE001 - 归档失败不阻断主流程
                logger.error("Overdrive 项目归档失败（非阻塞）: run={} error={}", run_id, exc)
            await runs.append_event(
                run_id,
                "run_completed",
                {
...
@shared_task(name="cygnusx.infrastructure.celery_app.tasks.overdrive.run_manager_job")
...
@shared_task(name="cygnusx.infrastructure.celery_app.tasks.overdrive.run_manager_review_job")
...
@shared_task(name="cygnusx.infrastructure.celery_app.tasks.overdrive.run_delivery_summary_job")
...
async def _run_delivery_summary_job(run_id: str) -> dict[str, object]:
    from cygnusx.application.services.agent_context_builder import AgentContextBuilder
    from cygnusx.application.services.overdrive_execution_service import DeliveryAssembler
    from cygnusx.application.services.overdrive_run_service import (
        OverdriveRunService,
        relative_overdrive_run_root,
    )
...
@shared_task(name="cygnusx.infrastructure.celery_app.tasks.overdrive.run_assistant_job")
...
async def _run_assistant_job(run_id: str, assistant_instance_id: str) -> dict[str, object]:
    ...
    from cygnusx.infrastructure.studio import studio_sandbox_manager
...
async def _is_registered_artifact_file(session_id: str, run_id: str, artifact: dict[str, object]) -> bool:
    ...
    from cygnusx.application.services.overdrive_run_service import overdrive_run_root
