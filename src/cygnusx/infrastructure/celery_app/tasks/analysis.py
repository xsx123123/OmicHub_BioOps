...
@shared_task(name="cygnusx.infrastructure.celery_app.tasks.analysis.run_snakemake")
def run_snakemake(
    snakefile: str,
    task_id: str,
    config_file: str = "",
...
async def _execute_snakemake(
    ...
    async with session_factory() as session:
        ...
        try:
            tracer = get_tracer("cygnusx.toolbox")
...
@shared_task(name="cygnusx.infrastructure.celery_app.tasks.analysis.cleanup_expired")
def cleanup_expired() -> dict:
    ...
    """
    from cygnusx.infrastructure.celery_app.tasks.storage import cleanup_expired as _real
