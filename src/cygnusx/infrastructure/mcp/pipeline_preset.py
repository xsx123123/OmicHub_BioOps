...
CYGNUSX_PIPELINES_SERVER_ID = uuid.uuid5(uuid.NAMESPACE_DNS, "cygnusx-pipelines.builtin")
CYGNUSX_PIPELINES_SERVER_NAME = "cygnusx-pipelines"
...
async def _pipeline_handler(
    ...
    **_kwargs: Any,
    ...
    if context is None:
        raise RuntimeError("cygnusx-pipelines 仅允许在带用户上下文的调用中执行")
...
def build_pipeline_preset() -> dict[str, Any]:
    return {
        "name": CYGNUSX_PIPELINES_SERVER_NAME,
        "description": "CygnusX 完整分析流程 MCP - RNA-seq、ATAC-seq 预检、提交、状态和结果",
        "transport": "builtin",
