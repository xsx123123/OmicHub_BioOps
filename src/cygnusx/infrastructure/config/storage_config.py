"""存储目录配置 —— 统一从 data/CygnusX.yaml 的 storage 段读取 data_root。
...
解析优先级（逐级降级，文件缺失/解析失败/字段为空时静默回退，绝不抛异常）：
  ...
  2) settings.storage_path           ← .env 的 STORAGE_PATH（默认 /data/cygnusx）
  3) 内置兜底 /data/cygnusx

复用 settings.site_content_yaml 指向的同一份 CygnusX.yaml，与 SiteContentService
...
_DEFAULT_DATA_ROOT = "/data/cygnusx"
