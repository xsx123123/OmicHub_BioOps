...
class Settings(BaseSettings):
    ...
    # Runtime 拆分灰度：关闭时保持当前 ChatService 直连路径。
    chat_runtime_refactor_enabled: bool = False
    # 单 Runtime 收敛逃生舱：默认所有非 Studio/非 MAS 普通聊天走 LangGraph 状态图；
    # 打开后全部回退手写循环（仅用于 LangGraph 运行时故障时的紧急回滚）。
    chat_force_legacy_runtime: bool = False
    # 事件顺序异常默认仅告警，避免 SSE 已开始后因观测校验中断用户回复；
    ...
    schedule_scan_batch_size: int = 100
    schedule_max_delivery_attempts: int = 5
    # ===== 任务队列传输 =====
    ...
    rocketmq_producer_group: str = "cygnusx-task-producer"
    rocketmq_consumer_group_prefix: str = "cygnusx-task-worker"
    rocketmq_topic_prefix: str = "cygnusx_task"
    ...
    # 429 限流单独的重试上限（独立计数，不占用瞬时重试额度）；优先尊重响应的 Retry-After 头，
    # 没有该头时退化为与瞬时重试相同的线性退避。上限刻意低于瞬时重试，避免限流时雪崩。
    llm_stream_rate_limit_retries: int = 2
    # ===== 联网检索优化 =====
    ...
    storage_s3_bucket: str = "cygnusx-data"
    ...
    mas_workspace_root: str = "/data/cygnusx"
    ...
    mas_apptainer_pipeline_root: str = "/opt/cygnusx/pipelines/RNAFlow"
    mas_scrna_docker_image: str = "cygnusx-analysis:scrna-v0.0.3dev"
    ...
    memory_extraction_model: str = "qdoubao-seed-evolving"
    ...
    # 无领域锚点时的通用能力匹配软校验：发现选派错配则请求 Manager 修复一次，
    # 修复未果仅记录违规不阻断流程；关闭即回退到纯 LLM 自由规划。
    overdrive_capability_check_enabled: bool = True
    ...
    # 设计见 ARCHITECTURE_DESIN/multi_agent_fanout_2026-07.md §4
    ...
    ebi_download_binary: str = "/data/cygnusx/bin/EBIDownload"
    ebi_download_yaml: str = "/data/cygnusx/bin/EBIDownload.yaml"
    enable_cloud_storage_download: bool = False  # 装好 ossutil/tosutil/obsutil 并配置凭据后再开
    cloud_ossutil_binary: str = "/data/cygnusx/bin/ossutil"
    cloud_tosutil_binary: str = "/data/cygnusx/bin/tosutil"
    cloud_obsutil_binary: str = "/data/cygnusx/bin/obsutil"
    ...
    snakemake_conda_prefix: str = "/data/cygnusx/.conda_envs"
    ...
    site_content_yaml: str = "data/CygnusX.yaml"  # 首页文案外置 YAML 路径（改文件即生效）
    ...
    # ===== 阿里云官方技能源（市场第二分组，可选）=====
    # 总开关：默认关闭，市场只展示平台内置源；置 true 后阿里云官方源分组才出现。
    # 关闭时同步/安装接口直接短路返回，不触网、不读缓存目录。
    aliyun_skills_enabled: bool = False
    ...
    aliyun_skills_cache_dir: str = "/data/cygnusx/skill_marketplace_aliyun"
    ...
    blast_db_root: str = "/data/cygnusx/blast/db"
    ...
    cygnusx_workflow_monitor_token: str = ""
    workflow_monitor_internal_url: str = "http://web:8000/api/v1/workflow-monitor"
    workflow_monitor_plugin_token_env: str = "CYGNUSX_WORKFLOW_MONITOR_TOKEN"
    ...
    # 30s 余量很薄；2026-08-26 协作室 agent-data 直答（长思考链）实测超 30s 被
    # 降级接管，故上调至 120s；直答带流式思考投影，等待期间用户可见进度
    agentteams_direct_timeout_seconds: float = 120.0
    # 协作室上下文读取面：正文默认共享，思考过程按需开启。
    room_context_message_limit: int = 20
    room_context_max_chars: int = 16_000
    room_context_include_thinking: bool = False
    room_messages_read_max_limit: int = 100

    # ===== Matrix Gateway（CygnusX 仅持有 Gateway 服务凭证，绝不保存 Matrix 凭证） =====
    ...
    # 宿主→容器挂载源（容器内固定为 /data/cygnusx，基因列表与结果 CSV 经此交换）
    ...
    deg_docker_image: str = "cygnusx-r-deg:v1"
    # 容器加入的 Docker 网络（与主栈 app 网络一致）
    deg_docker_network: str = "cygnusx_app_net"
    # 宿主→容器挂载源（容器内固定为 /data/cygnusx，输入与结果经此交换）
    deg_data_mount: str = "/data/cygnusx"
    ...
    # 故改用 storage_path（/data/cygnusx，容器与宿主共享且可写）下的
    # cygnusx_data/welcome/ 子目录，避免与任务数据混放。
    welcome_yaml: str = (
        "/data/cygnusx/cygnusx_data/welcome/welcome.yaml"  # 预生成欢迎词（纯字符串数组）
    )
    welcome_state_yaml: str = "/data/cygnusx/cygnusx_data/welcome/welcome_state.yaml"  # 轮询游标
    welcome_seed_threshold: int = 20  # 启动补齐目标条数

    # ===== 代码执行沙盒 =====
    sandbox_enabled: bool = True
    sandbox_image: str = "cygnusx-sandbox-copilot:v0.0.2dev"
    sandbox_docker_network: str = "cygnusx_default"
    ...
    sandbox_data_dir: str = "/data/cygnusx"
    sandbox_default_cpu: float = 2.0
    sandbox_default_memory: str = "4g"
    # ===== 沙盒安全基线（默认值对齐 Studio StudioSandboxConfig）=====
    # 容器运行用户（非 root，"uid:gid"）
    sandbox_container_user: str = "10001:10001"
    # 容器内 PID 上限（防 fork 炸弹）
    sandbox_pids_limit: int = 512
    # 只读 rootfs；可写需求由 /tmp 与交付/输入目录的 tmpfs 满足
    sandbox_read_only_rootfs: bool = True
    # seccomp：default=使用 daemon 内置默认 profile（不下发 security_opt）；
    # unconfined=显式禁用；其余值按字面 JSON profile 内容透传
    sandbox_seccomp_profile: str = "default"
    # /tmp 的 tmpfs 大小（nosuid,nodev,noexec，同 Studio 基线）
    sandbox_tmpfs_size: str = "512m"
    # 交付/输入目录 bind mount 的宿主机基目录。必须是 Web 容器与 Docker
    # daemon 共同可见的路径；不要使用 Web 容器私有的 /tmp，否则 daemon 会把
    # 不存在的 source 自动创建成 root:root 0755，导致 uid 10001 无法写入。
    sandbox_pool_host_dir: str = "/data/cygnusx/.sandbox-pool"
    ...
    # 外部 MCP server:tool 连续失败 3 次进入 degraded 后的熔断冷却窗口：
    # 窗口内调用直接短路走兜底链；窗口过后放行一次半开试探，成功恢复 healthy，失败重新冷却。
    mcp_circuit_breaker_cooldown_seconds: float = 60.0
    ...
    mcp_builder_default_model: str = "qdoubao-seed-evolving"  # 代码生成默认模型（与平台 agent 标准一致）
    ...
    # 是否将 span 持久化到应用内 JSONL（cygnusx.spans.json.log），供管理端 Trace 瀑布图查询。
    ...
    def validate_production_secrets(self) -> "Settings":
        ...
        if not self.workflow_monitor_ingest_token and self.cygnusx_workflow_monitor_token:
            self.workflow_monitor_ingest_token = self.cygnusx_workflow_monitor_token
