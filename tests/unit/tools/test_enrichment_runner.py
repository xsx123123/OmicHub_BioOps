...
from omichub.tools.enrichments.runner import EnrichmentDockerRunner, EnrichmentRunParams
...
def test_runner_passes_local_go_and_kegg_references_to_r_container() -> None:
    ...
    runner._settings = SimpleNamespace(
        enrichment_docker_network="omichub_app_net",
        enrichment_proxy_url="",
        enrichment_data_mount="/data/omichub",
        enrichment_memory="2g",
        enrichment_cpus=2.0,
        enrichment_docker_image="omichub-r-enrichment:v1",
    )
    params = EnrichmentRunParams(
        input_path="/data/omichub/users/u1/enrichments/tomato_project/t1/gene_list.txt",
        output_path="/data/omichub/users/u1/enrichments/tomato_project/t1/enrichment_result.csv",
        kegg_code="sly",
        id_type="ITAG4.1",
        go_obo="/data/omichub/omichub_data/reference/ITAG4.1/go-basic.obo",
        go_annotation="/data/omichub/omichub_data/reference/ITAG4.1/go.annot",
        kegg_id_map="/data/omichub/omichub_data/reference/ITAG4.1/kegg.id",
    )

    command = runner.build_command(params, "omichub-enrich-test")

    assert command[:3] == ["docker", "run", "--rm"]
    assert command[command.index("--network") + 1] == "omichub_app_net"
...
def test_runner_uses_docker_default_bridge_when_network_is_not_configured() -> None:
    ...
    params = EnrichmentRunParams(
        input_path="/data/omichub/users/u1/enrichments/project/t1/gene_list.txt",
        output_path="/data/omichub/users/u1/enrichments/project/t1/enrichment_result.csv",
        kegg_code="hsa",
        id_type="ENTREZID",
    )
...
def test_project_name_builds_safe_user_enrichment_directory() -> None:
    ...
    service._settings = SimpleNamespace(enrichment_data_mount="/data/omichub")
    ...
    assert work_dir == Path("/data/omichub/users/user-1/enrichments/Tomato_COP1_HY5/task-1")
