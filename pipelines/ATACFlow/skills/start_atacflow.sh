#!/bin/bash
# ATACFlow Enhanced Execution Script with Conda Environment Check
# Usage: ./start_atacflow.sh /path/to/config.yaml
# Version: 1.0
...
if [ -f "$PATH_CONFIG" ]; then
    ...
    ATACFLOW_ROOT=$(grep '^ATACFLOW_ROOT:' "$PATH_CONFIG" | cut -d' ' -f2 | tr -d '"')
...
if [ $# -eq 0 ]; then
    ...
    echo "2. Check if the ATACFlow conda environment exists"
    echo "3. Ask for user confirmation before activating environment"
    echo "4. Run ATACFlow analysis"
    exit 1
fi
...
if [ ! -d "$ATACFLOW_ROOT" ]; then
    ...
    echo "Please update ATACFLOW_ROOT in this script or in path_config.yaml"
    exit 1
fi

# ==================== Conda Environment Setup ====================
echo "=========================================="
echo "ATACFlow Analysis Pipeline - Environment Check"
echo "=========================================="
echo ""
