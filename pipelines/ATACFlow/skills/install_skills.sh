#!/bin/bash
# ATACFlow Skills Installer
# Installs ATACFlow skills to opencode/AI agent skills directory
...
if [ -d "$HOME/.claude/skills" ]; then
    TARGET_DIR="$HOME/.claude/skills/ATACFlow"
...
if [ -z "$TARGET_DIR" ] && [ -d "$HOME/.codex/skills" ]; then
    TARGET_DIR="$HOME/.codex/skills/ATACFlow"
    SKILLS_TYPE="Codex"
...
if [ -z "$TARGET_DIR" ] && [ -d "$HOME/.config/opencode" ]; then
    TARGET_DIR="$HOME/.config/opencode/skills/ATACFlow"
...
if [ -z "$TARGET_DIR" ]; then
    ...
    case $CHOICE in
        ...
        2)
            ...
            ;;
        3)
            echo "Please enter the target directory for installing ATACFlow skills:"
            read -p "Target directory: " USER_TARGET
            if [ -z "$USER_TARGET" ]; then
                echo "Error: No target directory specified"
                exit 1
            fi
            TARGET_DIR="$USER_TARGET/ATACFlow"
            SKILLS_TYPE="Custom"
            ;;
        *)
            echo "Invalid choice"
...
echo "=========================================="
echo "ATACFlow Skills Installer"
echo "=========================================="
...
echo "=========================================="
echo ""
echo "ATACFlow skills have been installed for $SKILLS_TYPE to:"
echo "  $TARGET_DIR"
echo ""
echo "What's included:"
echo "  - SKILL.md: Main skill definition"
echo "  - path_config.yaml: Path configuration"
echo "  - start_atacflow.sh: Enhanced startup script"
