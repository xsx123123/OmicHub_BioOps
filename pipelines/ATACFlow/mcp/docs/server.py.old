...
def setup_logging():
    ...
    # 获取logger
    logger = logging.getLogger("ATACFlowMCP")
    ...
    logger.info(f"=== ATACFlow MCP Server 启动 ===")
...
def init_database():
    ...
    db_path = db_dir / "atacflow_runs.db"
...
def get_db_connection():
    """获取数据库连接"""
    db_path = Path(__file__).parent / "data" / "atacflow_runs.db"
...
class ProjectConfig(BaseModel):
    ...
    mapping_tool: str = Field(
        default="chromap", description="Mapping tool (chromap or bowtie2)"
    )
    use_pooled_peaks: bool = Field(
        default=True, description="Use pooled peaks for DEG analysis"
    )
...
def list_supported_genomes() -> List[Dict[str, str]]:
    ...
    # 如果没有 mcp_genome_version，则回退到从 Bowtie2_index 读取
    elif "Bowtie2_index" in ref_config:
...
async def run_atacflow(
    ...
    try:
        if dry_run:
            ...
            message_parts.append(
                "🚀 ATACFlow pipeline has been successfully started in the background!"
            )
        ...
        logger.error(error_msg)
        logger.info("=== 工具完成: run_atacflow (失败) ===")
        return error_msg
    except Exception as e:
        logger.error(f"启动 ATACFlow 失败: {str(e)}", exc_info=True)
        logger.info("=== 工具完成: run_atacflow (失败) ===")
