...
```bash
# 需要先安装 Node.js
npx @modelcontextprotocol/inspector uv --directory /home/zj/pipeline/ATACFlow/mcp run server.py
```
...
创建 `/etc/systemd/system/atacflow-mcp.service`（需要 sudo）：
```ini
[Unit]
