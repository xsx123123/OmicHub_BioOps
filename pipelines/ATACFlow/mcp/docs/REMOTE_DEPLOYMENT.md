...
本指南说明如何将 ATACFlow MCP 服务从本地 stdio 模式转换为远程部署模式。
...
创建 `/etc/systemd/system/atacflow-mcp.service`：
```ini
[Unit]
Description=ATACFlow MCP Server
After=network.target

[Service]
Type=simple
User=zj
WorkingDirectory=/home/zj/pipeline/ATACFlow/mcp
...
sudo systemctl enable atacflow-mcp
sudo systemctl start atacflow-mcp
sudo systemctl status atacflow-mcp
```

### Step 3: 配置 Nginx 反向代理
创建 `/etc/nginx/sites-available/atacflow-mcp`：
```nginx
server {
    listen 80;
...
启用配置：
```bash
sudo ln -s /etc/nginx/sites-available/atacflow-mcp /etc/nginx/sites-enabled/
sudo nginx -t
