#!/usr/bin/env python3
"""
ATACFlow MCP Server - Main Entry Point
...
def setup_complete_project_tool(
    ...
    mapping_tool: str = "chromap",
) -> Dict[str, Any]:
    """One-stop setup for complete ATACFlow project"""
...
async def run_atacflow_tool(
    ...
    """Run ATACFlow pipeline (Asynchronous / Detached)"""
    return await run_atacflow(
...
def list_runs_tool(
    ...
    """List ATACFlow project run records"""
...
@mcp.tool(name="atacflowGenerateConfigFile")
def atacflow_generate_config_file_old(config: dict, output_path: str) -> str:
    """Backward compatibility: accepts old-style config dict"""
    logger.info("使用向后兼容的 atacflowGenerateConfigFile (旧格式)")
    try:
        mapped_config = {
            ...
            "mapping_tool": config.get("mapping_tool", "chromap"),
...
@mcp.tool(name="atacflowValidateConfig")
def atacflow_validate_config_old(config_path: str) -> Dict[str, Any]:
    """Backward compatibility alias"""
...
@mcp.tool(name="atacflowRunRnaflow")
def atacflow_run_atacflow_old(
    config_path: str,
    cores: int = 20,
    dry_run: bool = False,
    skip_resource_check: bool = False,
    ...
    """Backward compatibility alias"""
    return run_atacflow(config_path, cores, dry_run, skip_resource_check)


@mcp.tool(name="atacflowCheckSnakemakeStatusTool")
def atacflow_check_snakemake_status_tool(run_id: str = None) -> Dict[str, Any]:
    """Backward compatibility alias"""
    return check_snakemake_status(run_id)


@mcp.tool(name="atacflowCheckSystemResourcesTool")
def atacflow_check_system_resources_tool() -> Dict[str, Any]:
    """Backward compatibility alias"""
    return check_system_resources()


@mcp.tool(name="atacflowCheckCondaEnvironmentTool")
def atacflow_check_conda_environment_tool(
...
@mcp.tool(name="atacflowListRunsTool")
def atacflow_list_runs_tool(
    project_name: str = None, status: str = None, limit: int = 50
) -> List[Dict[str, Any]]:
    """Backward compatibility alias"""
    return list_runs(project_name, status, limit)
...
@mcp.tool(name="atacflowGetRunDetailsTool")
def atacflow_get_run_details_tool(run_id: str) -> Dict[str, Any]:
    """Backward compatibility alias"""
    return get_run_details(run_id)


@mcp.tool(name="atacflowGetRunStatisticsTool")
def atacflow_get_run_statistics_tool(
    start_date: str = None, end_date: str = None
) -> Dict[str, Any]:
    """Backward compatibility alias"""
    return get_run_statistics(start_date, end_date)
...
@mcp.tool(name="atacflowCheckProjectNameConflictTool")
def atacflow_check_project_name_conflict_tool(project_name: str) -> Dict[str, Any]:
    """Backward compatibility alias"""
    return check_project_name_conflict(project_name)


@mcp.tool(name="atacflowGetSnakemakeLogTool")
def atacflow_get_snakemake_log_tool(run_id: str, lines: int = 50) -> Dict[str, Any]:
    """Backward compatibility alias"""
    return get_snakemake_log(run_id, lines)


@mcp.tool(name="atacflowListSupportedGenomesTool")
def atacflow_list_supported_genomes_tool() -> List[Dict[str, str]]:
    """Backward compatibility alias"""
    return list_supported_genomes()


@mcp.tool(name="atacflowGetConfigTemplateTool")
def atacflow_get_config_template_tool(template_type: str = "standard") -> str:
    """Backward compatibility alias"""
    return get_config_template(template_type)


@mcp.tool(name="atacflowCreateProjectStructureTool")
def atacflow_create_project_structure_tool(project_root: str) -> Dict[str, Any]:
    """Backward compatibility alias"""
    return create_project_structure(project_root)


@mcp.tool(name="atacflowGenerateConfigFileTool")
def atacflow_generate_config_file_tool(config: ProjectConfig, output_path: str) -> str:
    """Backward compatibility alias"""
    return generate_config_file(config, output_path)


@mcp.tool(name="atacflowCreateSampleCsvTool")
def atacflow_create_sample_csv_tool(
...
@mcp.tool(name="atacflowCreateContrastsCsvTool")
def atacflow_create_contrasts_csv_tool(
...
@mcp.tool(name="atacflowValidateConfigTool")
def atacflow_validate_config_tool(config_path: str) -> Dict[str, Any]:
    """Backward compatibility alias"""
    return validate_config(config_path)


@mcp.tool(name="atacflowGetProjectStructureTool")
def atacflow_get_project_structure_tool() -> Dict[str, Any]:
    """Backward compatibility alias"""
    return get_project_structure()


@mcp.tool(name="atacflowSetupCompleteProjectTool")
def atacflow_setup_complete_project_tool(
    project_root: str,
    project_name: str,
    genome_version: str,
    species: str,
...
@mcp.tool(name="atacflowRunSimpleQcAnalysisTool")
def atacflow_run_simple_qc_analysis_tool(
    project_root: str,
    genome_version: str,
    species: str,
    project_name: Optional[str] = None,
...
def setup_new_project(project_name: str = "My_RNA_Project") -> str:
    ...
    return f"""I want to start a new RNA-seq analysis project named "{project_name}" using ATACFlow. 
...
4. Then run the analysis with `run_atacflow`
"""
...
if __name__ == "__main__":
    logger.info(f"Starting ATACFlow MCP Server...")
    logger.info(f"ATACFlow Root: {ATACFLOW_ROOT}")
