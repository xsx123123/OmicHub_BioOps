...
**问题:** 原代码中的 `atacflowRunRnaflow` 工具直接调用 `run_atacflow()` 异步函数，返回 coroutine 对象而非实际结果。

**解决方案:**
- 移除了原 `atacflowRunRnaflow` 等旧版工具的直接同步调用
...
**优化措施:**
- 保留关键向后兼容工具：`atacflowGenerateConfigFile`, `atacflowValidateConfig`
...
**实现功能:**
- `run_atacflow()` dry run 添加 `asyncio.wait_for(timeout=60秒)`
...
**更新的工具文档:**
- `run_atacflow_tool` - 详细说明异步特性、监控方法、推荐工作流
...
- `atacflowGenerateConfigFile` (旧格式支持)
- `atacflowValidateConfig` (旧格式支持)

✅ **参数格式兼容**:
- `atacflowGenerateConfigFile` 支持旧式 dict 配置
- `atacflowValidateConfig` 返回兼容格式
