# ATACFlow MCP Server (v0.2.0 Optimized)

这是一个基于 [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) 构建的高性能组学分析服务端，专门为 ATAC-seq 分析流程设计。本项目通过 **[uv](https://docs.astral.sh/uv/)** 进行环境管理，采用模块化分层架构，支持本地 Stdio 和远程部署模式。
...
```json
{
  "mcpServers": {
    "atacflow": {
      "command": "uv",
      "args": ["--directory", "/home/zj/pipeline/ATACFlow/mcp", "run", "main.py"]
    }
  }
}
```
  ...
  "mcpServers": {
    "atacflow": {
      ...
      "args": [
        "-p", "22",
        "zj@your-server-ip",
        "cd /home/zj/pipeline/ATACFlow/mcp && /home/zj/.local/bin/uv run python main.py"
      ]
    }
  }
}
...
| **Exec** | `run_atacflow` | **异步提交流程**，返回 `job_id` |
...
5.  **提交流程**: 调用 `run_atacflow(config_path="...", dry_run=True)` 进行演练。
...
## 📝 维护与日志
- **服务器日志**: `/home/zj/pipeline/ATACFlow/mcp/logs/mcp/`
- **任务日志**: 每个项目的 `01.workflow/atacflow_run.log`
- **数据库**: `/home/zj/pipeline/ATACFlow/mcp/data/atacflow_runs.db`

---
**ATACFlow MCP** - *让组学分析触手可及* 🚀