...
async def create_project_structure_tool(project_root: str) -> Dict[str, Any]:
    ...
    return await loop.run_in_executor(None, create_project_structure, project_root)
...
async def validate_config_tool(config_path: str) -> Dict[str, Any]:
    ...
    """
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, validate_config, config_path)
...
async def setup_complete_project_tool(
    ...
    Does NOT execute analysis - use run_atacflow_tool for that.
    ...
    Args:
        ...
        mapping_tool: Library type, typically "fr-firststrand"
...
async def run_simple_qc_analysis_tool(
    ...
    Use run_atacflow_tool after reviewing the generated config.
...
async def run_atacflow_tool(
    ...
    Submit ATACFlow pipeline for execution. Returns immediately with a job_id.
...
@mcp.tool()
async def check_conda_environment_tool(
    ...
    """
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, check_conda_environment, env_name)
...
async def check_system_resources_tool() -> Dict[str, Any]:
    ...
    """
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, check_system_resources)
...
async def list_runs_tool(
    ...
    """
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, list_runs, project_name, status, limit)
...
async def get_run_details_tool(run_id: str) -> Dict[str, Any]:
    ...
    """
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, get_run_details, run_id)
...
async def get_run_statistics_tool(
    ...
    """
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, get_run_statistics, start_date, end_date)
...
async def check_project_name_conflict_tool(project_name: str) -> Dict[str, Any]:
    ...
    """
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, check_project_name_conflict, project_name)
...
async def check_snakemake_status_tool(run_id: str = None) -> Dict[str, Any]:
    ...
    """
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, check_snakemake_status, run_id)
...
async def get_snakemake_log_tool(run_id: str, lines: int = 50) -> Dict[str, Any]:
    ...
    """
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, get_snakemake_log, run_id, lines)
...
def atacflow_generate_config_file_old(config: dict, output_path: str) -> str:
    ...
    try:
        mapped_config = {
            ...
            "call_peaks": config.get("call_peaks", not config.get("only_qc", False)),
            "motif_analysis": config.get("motif_analysis", False),
            "ataqv": config.get("ataqv", True),
...
async def get_complete_config_template() -> str:
    """
    Resource: Complete ATACFlow configuration template
...
async def get_skill_documentation() -> str:
    """
    Resource: ATACFlow SKILL.md documentation
...
def setup_new_project(project_name: str = "My_ATAC_Project") -> str:
    """
    Guide AI to set up a new ATACFlow analysis project

    Use this prompt when starting a new ATAC-seq analysis.
    """
...
def troubleshoot_failure(log_path: str = "01.workflow/atacflow_run.log") -> str:
    ...
    Use this prompt when ATACFlow execution fails.
    """
...
4. Then run the analysis with `run_atacflow_tool`
"""
...
def prepare_publication_report() -> str:
    ...
    Use this after completing ATACFlow analysis.
    """
...
4. Provide a structure for the "Methods" section of my paper based on current ATACFlow workflow.
"""
...
def _print_startup_info():
    ...
    logger.info("=" * 60)
    logger.info("ATACFlow MCP Server v0.2.0 (Optimized)")
    logger.info("=" * 60)
