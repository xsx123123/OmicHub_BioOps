...
async fn run_download(args: &DownloadArgs, cli: &Cli) -> Result<()> {
    ...
    if args.dry_run {
        ...
        if args.progress_port.is_some() {
            info!("🏜️  Simulating progress for {} runs...", processed.len());
            {
            ...
            for record in &processed {
                ...
                // downloading
                {
                    ...
                    if let Some(rp) = map.get_mut(run_id) {
                        ...
                        rp.download.update(sra_size, sra_size);
                        rp.recalculate_overall();
                    }
                }
                info!("  📥 [{}] downloading → done", run_id);
                ...
                // extracting
                {
                    ...
                    if let Some(rp) = map.get_mut(run_id) {
                        ...
                        let extract_bytes = (sra_size as f64 * 3.0) as u64;
                        rp.extraction.update(extract_bytes, extract_bytes);
                        rp.recalculate_overall();
                    }
                }
                info!("  📂 [{}] extracting → done", run_id);
                tokio::time::sleep(Duration::from_secs(2)).await;
                ...
                // compressing
                {
                    ...
                    if let Some(rp) = map.get_mut(run_id) {
                        ...
                        let compress_bytes = (sra_size as f64 * 3.0) as u64;
                        rp.compression.update(compress_bytes, compress_bytes);
                        rp.recalculate_overall();
                    }
                }
                info!("  📦 [{}] compressing → done", run_id);
                tokio::time::sleep(Duration::from_secs(2)).await;
            ...
            info!("🏜️  Dry Run progress simulation done. Waiting 5s for external polling...");
