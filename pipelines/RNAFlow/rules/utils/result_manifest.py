...
def collect_delivered_files(deliver_dir) -> List[Dict]:
    ...
    # Engine is resolved by the snakefile (rules/utils/deg_method.py): the
    # delivery layout mirrors it as 05_DEG/DESEQ2 (default) or 05_DEG/EDGER
    # (1v1 / no-replicate runs). Detect what actually exists on disk.
    deg_dirname = next(
        (d for d in ("DESEQ2", "EDGER") if (root / "05_DEG" / d).is_dir()),
        "DESEQ2",
    )
    deg_dir = root / "05_DEG" / deg_dirname
    if deg_dir.is_dir():
        ...
        add(f"05_DEG/{deg_dirname}/All_Contrast_DEG_Statistics.csv", "全部比较组 DEG 统计", "csv", "deg")
...
def build_comparisons(all_contrasts: Optional[List[str]], deg_dirname: str = "DESEQ2") -> List[Dict]:
    ...
    deg_dirname selects the engine sub-directory (DESEQ2 default, EDGER for
    no-replicate runs routed to edgeR).
    """
    comparisons = []
        ...
        comparisons.append(
            {
                ...
                "control": ctrl,
                "treat": treat,
                "result_path": f"05_DEG/{deg_dirname}/{legacy_name}_DEG.csv",
            }
        )
    return comparisons
...
def build_success_manifest(
    ...
    comparisons = build_comparisons(
        all_contrasts,
        deg_dirname=str(
            config.get("parameter", {}).get("DEG", {}).get("METHOD_RESOLVED", "deseq2")
        ).upper(),
    )
