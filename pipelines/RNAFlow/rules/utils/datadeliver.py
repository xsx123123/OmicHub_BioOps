...
def Deg(samples: Dict = None, data_deliver: List = None, config: Dict = None) -> List:
    """
    ...
    Args:
        samples: Dictionary of sample information
        data_deliver: List of deliverable files to extend
        config: Pipeline config; parameter.DEG.METHOD_RESOLVED (set by the
            snakefile via rules/utils/deg_method.py) selects the engine and
            therefore the result sub-directory (06.DEG/DESEQ2 | 06.DEG/EDGER).
    ...
    method = str(
        (config or {}).get('parameter', {}).get('DEG', {}).get('METHOD_RESOLVED', 'deseq2')
    ).lower()
    data_deliver.append(f"06.DEG/{method.upper()}")
