#!/usr/bin/env python3
"""Configuration loading utilities for cygnusxtools.

This module follows the same package-resource pattern as the gpse utilities: the
default YAML files live inside ``cygnusxtools.config`` and can be overridden by
...
logger = logging.getLogger("cygnusxtools.config")

PROJECT_CONFIG_FILES = ("cygnusxtools.yaml", "cygnusxtools.local.yaml")


def _load_yaml(config_name: str) -> dict[str, Any]:
    config_path = resources.files("cygnusxtools.config") / f"{config_name}.yaml"
