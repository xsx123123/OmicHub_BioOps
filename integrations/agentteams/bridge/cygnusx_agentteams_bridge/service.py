...
from .audit import OPERATIONAL_EVENT_TYPES, STREAM_EVENT_TYPES, AuditStore
from .case_store import CaseStore
from .client import GatewayClient, CygnusXClient
...
class BridgeService:
    ...
    async def _flow_allowed_after_refresh(self, flow_id: str) -> bool:
        """Refresh a stale capability snapshot before rejecting a flow."""
        if flow_id in self._settings.allowed_flows():
            return True
        try:
            await self.refresh_capabilities()
        except Exception:
            logger.warning("capability refresh failed while validating flow=%s", flow_id, exc_info=True)
        return flow_id in self._settings.allowed_flows()
    ...
    # ------------------------------------------------------------------
    # 房间命名空间归档/恢复（协作室房间事件GC与归档口径 §2，归档 ≠ 删除）
    # ------------------------------------------------------------------
    async def archive_room_namespace(
        self, room_id: str, actor: str, reason: str | None = None
    ) -> dict[str, Any]:
        """Archive a room namespace event stream to the MinIO cold prefix.

        口径 §2：先向在线事件流末尾追加 room.archived 审计事件（操作者/原因/
        冷存位置/事件量快照），随事件卷一并迁往 ``archive/rooms/<room_id>/``；
        归档后在线审计链查询不再加载该卷，business 写入被显式拒绝（见
        record_evidence 的只读拦截）。已归档的命名空间重复调用幂等返回。
        """
        case_id = self._room_namespace_case_id(room_id)
        case = await self._cases.get(case_id)
        if case.record_kind != "room_namespace":
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="Target case_id is not a room namespace",
            )
        if not self._audit.supports_room_archive:
            raise HTTPException(
                status_code=status.HTTP_501_NOT_IMPLEMENTED,
                detail="Room namespace archiving requires the MinIO persistence layer",
            )
        if self._audit.room_namespace_archived(room_id):
            return {
                "room_id": room_id,
                "case_id": case_id,
                "archived": True,
                "already_archived": True,
                "archive_prefix": MinioBridgeStorage.archived_events_prefix(room_id),
            }
        events = await self._audit.list_events(case_id)
        business_events = [
            event
            for event in events
            if event.get("event_type") not in STREAM_EVENT_TYPES
            and event.get("event_type") not in OPERATIONAL_EVENT_TYPES
        ]
        last_business_at = self._last_business_recorded_at(business_events)
        await self._audit.record(
            case_id=case_id,
            actor=actor,
            ...
            payload={
                ...
                "last_business_recorded_at": (
                    last_business_at.isoformat() if last_business_at is not None else None
                ),
            },
        )
        migration = await self._audit.archive_room_events(case_id, room_id)
        return {
            "room_id": room_id,
            "case_id": case_id,
            "archived": True,
    ...
    async def unarchive_room_namespace(self, room_id: str, actor: str) -> dict[str, Any]:
        """Restore an archived room namespace: cold volumes move back, then audit.

        口径「恢复」：冷存事件卷回搬后追加 room.unarchived 审计事件；恢复后
        read_events 能完整读出归档前全部事件 + room.archived + room.unarchived，
        审计链完整可重放。
        """
        case_id = self._room_namespace_case_id(room_id)
        case = await self._cases.get(case_id)
        ...
        if not self._audit.room_namespace_archived(room_id):
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Room namespace is not archived",
            )
        restoration = await self._audit.unarchive_room_events(case_id, room_id)
        await self._audit.record(
            case_id=case_id,
            actor=actor,
            event_type="room.unarchived",
            payload={
                "room_id": room_id,
                "restored_from": restoration["restored_from"],
                "restored_object_count": len(restoration["restored_objects"]),
            },
        )
        return {
            "room_id": room_id,
    ...
    async def archive_stale_room_namespaces(self, actor: str) -> dict[str, Any]:
        """Auto-archive scan: archive room namespaces idle beyond room_archive_days.

        口径 §2 归档触发：以最后一条 business 事件的 recorded_at 判定活动
        （operational/stream 不算活动）；已归档的跳过，单房间失败不阻断扫描。
        """
        days = self._settings.room_archive_days
        if days <= 0:
            return {
        ...
        for case in await self._cases.list():
            if case.record_kind != "room_namespace":
                continue
            summary["scanned"] += 1
            room_id = case.case_id[len(ROOM_NAMESPACE_ID_PREFIX):]
            if self._audit.room_namespace_archived(room_id):
                summary["skipped"] += 1
                continue
            events = await self._audit.list_events(case.case_id)
            last_business_at = self._last_business_recorded_at(events)
            if last_business_at is None or last_business_at >= cutoff:
                summary["skipped"] += 1
                continue
            try:
                await self.archive_room_namespace(
                    room_id, actor, reason=f"inactive_for_{days}d(auto)"
                )
            except Exception:
                logger.warning(
                    "room namespace auto-archive failed for %s", case.case_id, exc_info=True
                )
                summary["failed"] += 1
                continue
            summary["archived"] += 1
    ...
    @staticmethod
    def _room_namespace_case_id(room_id: str) -> str:
        if not room_id or "/" in room_id:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Invalid room_id"
            )
        return f"{ROOM_NAMESPACE_ID_PREFIX}{room_id}"

    def _last_business_recorded_at(self, events: list[dict[str, Any]]) -> datetime | None:
        """Latest recorded_at among business events (stream/operational不算活动)。"""
        latest: datetime | None = None
        for event in events:
            event_type = str(event.get("event_type") or "")
            if event_type in STREAM_EVENT_TYPES or event_type in OPERATIONAL_EVENT_TYPES:
                continue
            parsed = self._parse_datetime(event.get("recorded_at"))
            if parsed is not None and (latest is None or parsed > latest):
                latest = parsed
        return latest

    def _require_room_namespace_writable(self, case: CaseRecord, event_type: str) -> None:
        """口径 §2.2：已归档房间命名空间只读，business 事件写入显式 409 拒绝。

        瞬态 stream 与 operational 计数不受影响（它们本就不进持久事件卷）。
        """
        if case.record_kind != "room_namespace":
            return
        if event_type in STREAM_EVENT_TYPES or event_type in OPERATIONAL_EVENT_TYPES:
        ...
        if self._audit.room_namespace_archived(room_id):
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Room namespace is archived; unarchive it before recording events",
            )

    async def reconcile_case(self, case_id: str, actor: str) -> CaseRecord:
        """Advance a submitted Case only from observed CygnusX task state.
        ...
        try:
            try:
                consult_payload: dict[str, Any] = {
                    ...
                    "work_item_id": work_item_id,
                    "trace_id": work_item.trace_id or request.trace_id,
                    "execution_mode": request.execution_mode,
                }
                # 产物血缘（F1）：Worker 声明的 source_refs/执行摘要原样透传到平台登记路径。
                if request.source_refs is not None:
                    consult_payload["source_refs"] = [
                        ref.model_dump(mode="json") for ref in request.source_refs
                    ]
                if request.execution_summary is not None:
                    consult_payload["execution_summary"] = request.execution_summary
                result = await self._gateway.consult(consult_payload)
    ...
    async def _accept_plan_result(
        ...
        try:
            ...
            if not await self._flow_allowed_after_refresh(task.flow_id):
    ...
    async def _handle_plan_validation_failure(
        ...
        """计划校验失败后的自愈回路：自动重试 `max_attempts` 次内反馈给 Planner 重试；
        自动重试耗尽后进入 `waiting_for_correction` 等待用户修正，最多
        `case.max_correction_rounds` 轮；用户修正轮次也耗尽后进入显式终态
        `planning_failed`，避免无限硬锁。
        """
        ...
        # 自动重试已耗尽：进入用户修正轮次或终态。
        next_round = case.correction_round_count + 1
        if next_round <= case.max_correction_rounds:
            if case.status == "planning_running":
                await self._cases.transition(case_id, "waiting_for_correction")
            await self._cases.bump_correction_round(case_id)
            await self._audit.record(
                case_id=case_id,
                actor=actor,
                ...
                payload={
                    ...
                    "max_correction_rounds": case.max_correction_rounds,
                    "next_actions": ["revise", "cancel"],
                },
            )
            await self._audit.record(
                case_id=case_id,
        ...
        # 用户修正轮次也耗尽：显式终态 planning_failed，防止硬锁。
        if case.status != "planning_failed":
            await self._cases.transition(case_id, "planning_failed")
        await self._audit.record(
            case_id=case_id,
            actor=actor,
            ...
            payload={
                ...
                "attempt": case.planning_retry_count,
                "max_attempts": max_attempts,
                "correction_round": case.correction_round_count,
                "max_correction_rounds": case.max_correction_rounds,
                "next_actions": ["cancel"],
            },
        )
    ...
    async def list_flows(self) -> dict[str, Any]:
        ...
        if not isinstance(items, list):
            raise HTTPException(status_code=502, detail="CygnusX returned malformed flow list")
    ...
    async def preflight(self, request: PreflightRequest, actor: str) -> PreflightResponse:
        ...
        if not await self._flow_allowed_after_refresh(request.flow_id):
        ...
        if case.status in {"received", "waiting_for_correction"}:
            if case.status == "waiting_for_correction":
                # 用户提交修正：开启新一轮自动重试预算。
                await self._cases.reset_planning_retry_count(request.case_id)
    ...
    async def submit_task(self, request: SubmitTaskRequest, actor: str) -> TaskReceipt:
        if not await self._flow_allowed_after_refresh(request.task.flow_id):
    ...
    async def retry_case_submission(
        ...
        retry_item = WorkItemRecord(
            ...
            objective="重试已获人工批准且保持冻结输入不变的 CygnusX 分析任务。",
    ...
    async def _submit_verified_task(
        ...
        *,
        ...
        if not omic_task_id:
            raise HTTPException(status_code=502, detail="CygnusX submit response has no task ID")
        receipt = TaskReceipt(
            case_id=case.case_id,
        ...
        if work_item_id:
            await self._update_bound_work_item(
                ...
                summary=f"CygnusX task {omic_task_id} submitted",
            )
        return receipt
    ...
    async def quality_gate(
        ...
        if request.artifact_hashes and not request.source_refs:
            # 产物血缘（F1）硬约束：上报产物必须携带 source_refs，缺失即拒绝登记，
            # 回写 room.artifact_rejected 审计事件（例外显式化，不静默兜底），
            # Case 状态机不发生任何迁移。
            await self._audit.record(
                case_id=case.case_id,
                actor=actor,
                event_type="room.artifact_rejected",
                payload={
                    "reason": "missing_source_refs",
                    "task_id": task_id,
                    "work_item_id": request.work_item_id,
                    "artifact_paths": sorted(request.artifact_hashes),
                    "detail": "上报产物缺少 source_refs（输入产物 id 列表 + 执行摘要），登记被拒绝。",
                },
            )
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="Artifact registration rejected: source_refs are required when artifact_hashes are reported",
            )
    ...
    async def close_case(self, request: CaseCloseRequest, actor: str) -> CaseCloseResponse:
        ...
        # 产物血缘（F1）：Bridge 侧血缘事实来自质量门事件声明的 source_refs，
        # 随交付 manifest 一并输出；权威版本行在平台 DB（artifact_version_lineage）。
    ...
    async def record_evidence(
        ...
        """
        case = await self._cases.get(case_id)
        # 归档口径 §2.2：已归档房间命名空间的 business 事件写入显式拒绝。
        self._require_room_namespace_writable(case, request.event_type)
