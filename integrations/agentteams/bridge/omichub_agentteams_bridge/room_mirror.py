...
OmicHub 建房后经 Case 级证据事件 ``room.created``（payload.room_id）把
...
_MATRIX_USER_IDENTITY_PREFIX = "omichub-user-"
...
def matrix_identity_for_requester(requester_ref: str) -> str:
    """平台用户 → Matrix 身份映射；清洗失败时回退共享 omichub-user 静态身份。"""
    slug = "".join(
...
class AuditRoomMirror:
    ...
    def _render(event: dict[str, Any]) -> tuple[str, str, dict[str, Any]]:
        ...
        if event_type == USER_MESSAGE_EVENT_TYPE:
            ...
            # 平台用户以独立 Matrix 账号发言（omichub-user-<slug>），Element 侧可区分发言人。
