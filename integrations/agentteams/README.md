...
- `bridge/`: independently deployable REST adapter; it never imports CygnusX internals.
...
authenticated CygnusX server-side proxy. Shared production state uses Redis while preserving the
same Bridge contracts and never reaching into CygnusX services or its database.

The browser never receives a Bridge secret or invokes Worker operations. CygnusX filters Case
...
CygnusX demo task payload; it is not a substitute for human approval in production.
