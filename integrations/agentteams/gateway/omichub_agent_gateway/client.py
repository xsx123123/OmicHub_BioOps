"""HTTP-only adapter for OmicHub's controlled professional consultation endpoint."""
...
class OmicHubConsultationClient:
    ...
    async def consult(self, request: ConsultationRequest) -> UpstreamConsultationResponse:
        ...
        response = await self._client.post(
            self._settings.omichub_consultation_path,
            json=payload,
            headers=headers,
        )
    ...
    async def capabilities(self) -> dict:
        response = await self._client.get(
            ...
            headers=self._auth_headers() | {"X-Integration-Token": self._settings.omichub_integration_token},
        )
        response.raise_for_status()
    ...
    def _auth_headers(self) -> dict[str, str]:
        if self._settings.omichub_integration_token:
            return {"X-Integration-Token": self._settings.omichub_integration_token}
        return {}
