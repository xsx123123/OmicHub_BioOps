...
const TERMINAL_HEIGHT_KEY = 'cygnusx:studio:terminal-height'
...
.editor-tabs { height: 38px; display: flex; align-items: stretch; border-bottom: 1px solid var(--studio-border, #e8e8f2); background: color-mix(in srgb, var(--studio-card, #fff) 88%, transparent); backdrop-filter: blur(18px); }
...
.editor-tab.preview span { font-style: italic; }.editor-tab.active { color: var(--studio-text, #2b2b3d); background: var(--studio-card, #fff); box-shadow: inset 0 2px var(--studio-primary, #6c5ce7); }.editor-tab:active,.new-tab:active { transform: scale(.97); }
...
.editor-statusbar { height: 24px; display: flex; align-items: center; gap: 12px; padding: 0 10px; color: var(--studio-text-sub, #8a8aa3); background: var(--studio-bg, #f8f7fc); border-top: 1px solid var(--studio-border, #e8e8f2); font-size: 10px; }.status-dot { width: 6px; height: 6px; border-radius: 50%; background: #aaa; }.status-dot.running { background: #2ecc71; box-shadow: 0 0 0 3px rgba(46,204,113,.12); }
.editor-empty { flex: 1; display: grid; place-content: center; justify-items: center; padding: 32px; text-align: center; background: radial-gradient(circle at 50% 35%, var(--studio-primary-soft, #f0edff), var(--studio-card, #fff) 48%); }.editor-empty h3 { margin: 8px 0 4px; }.editor-empty p { max-width: 420px; color: var(--studio-text-sub, #8a8aa3); }.star-orbit { font-size: 38px; color: var(--studio-primary, #6c5ce7); filter: drop-shadow(0 8px 18px rgba(108,92,231,.25)); }
