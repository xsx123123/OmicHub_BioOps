...
describe('formatUserPath', () => {
  it('only exposes the path below a user home directory', () => {
    expect(formatUserPath('/data/cygnusx/users/cb79a200-b2ca-441f-9a42-d3417fbfa89d/raw_data/raw-data/PRJNA1478012'))
      .toBe('/raw_data/raw-data/PRJNA1478012')
  })

  it('represents a user home itself as the visible root', () => {
    expect(formatUserPath('/data/cygnusx/users/cb79a200-b2ca-441f-9a42-d3417fbfa89d')).toBe('/')
  })

  it('does not rewrite platform or temporary paths', () => {
    expect(formatUserPath('/data/cygnusx/cygnusx_data/db/Ath/TAIR10')).toBe('/data/cygnusx/cygnusx_data/db/Ath/TAIR10')
    expect(formatUserPath('/tmp/cygnusx/task-1')).toBe('/tmp/cygnusx/task-1')
  })
...
describe('redactUserHomePaths', () => {
  it('removes every embedded user-home prefix from status text', () => {
    expect(redactUserHomePaths(
      'Failed writing /data/cygnusx/users/user-a/raw_data/a.txt; retry /data/cygnusx/users/user-a/raw_data/b.txt.',
