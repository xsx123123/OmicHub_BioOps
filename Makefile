...
dev: ## 启动后端开发服务器
	uv run uvicorn cygnusx.main:app --reload --host 0.0.0.0 --port 8000

dev-worker: ## 启动 Celery Worker
	uv run celery -A cygnusx.infrastructure.celery_app.celery worker --loglevel=info

dev-rocketmq-worker: ## 启动 RocketMQ Worker（需先启动 RocketMQ broker）
	uv run python -m cygnusx.infrastructure.task_queue.rocketmq_worker --queues analysis,blast_search,blast_db_build,phylo_tree,phylo_tree_highmem,mas_apptainer

dev-beat: ## 启动 Celery Beat
	uv run celery -A cygnusx.infrastructure.celery_app.celery beat --loglevel=info

dev-flower: ## 启动 Flower 监控
	uv run celery -A cygnusx.infrastructure.celery_app.celery flower --port=5555
...
test-cov: ## 运行测试并生成覆盖率报告
	uv run pytest tests/ --cov=src/cygnusx --cov-report=term-missing --cov-report=html
...
COMPOSE_MAIN_AGENTTEAMS := $(COMPOSE_MAIN) -f deploy/agentteams/docker-compose.cygnusx-proxy.yml
COMPOSE_PGVECTOR_AGENTTEAMS := $(COMPOSE_PGVECTOR) -f deploy/agentteams/docker-compose.cygnusx-proxy.yml
...
# 主栈 web/beat/flower 经 CYGNUSX_BUILD_* 传入镜像（/health 暴露），
# AgentTeams Bridge 经 BRIDGE_BUILD_* 传入（/healthz 暴露）；compose 内缺省 unknown。
GIT_SHA := $(shell git rev-parse --short HEAD 2>/dev/null || echo unknown)
BUILD_TIME := $(shell date -u +%Y-%m-%dT%H:%M:%SZ)
export CYGNUSX_BUILD_SHA ?= $(GIT_SHA)
export CYGNUSX_BUILD_TIME ?= $(BUILD_TIME)
...
IMAGE_TAG ?= $(GIT_SHA)
export IMAGE_TAG
DATA_ROOT ?= $(if $(ENV_CYGNUSX_DATA_ROOT),$(ENV_CYGNUSX_DATA_ROOT),/data/cygnusx)
PUID ?= $(shell id -u)
PGID ?= $(shell id -g)
REDIS_PASSWORD ?= $(if $(ENV_REDIS_PASSWORD),$(ENV_REDIS_PASSWORD),cygnusx)
...
docker-up-matrix-dev: ## 启动本机开发用 Matrix(Synapse)+Element 栈（协作室建房链路依赖，独立于主栈/AgentTeams 栈）
	@echo "=> 启动 matrix-dev（cygnusx-matrix-dev-synapse / element）..."
...
docker-runtime-dirs: ## 准备关键运行目录与非 root 容器写入权限（docker-up/reload 自动执行）
	@echo "=> 准备运行目录权限..."
	@docker run --rm -v $(DATA_ROOT):/data-root python:3.11-slim sh -c 'mkdir -p /data-root/logs/app /data-root/logs/celery/tasks /data-root/logs/nginx /data-root/logs/snakemake /data-root/users /data-root/uploads /data-root/.tmp /data-root/.conda_envs /data-root/cygnusx_data/_pgdata /data-root/cygnusx_data/_redis /data-root/cygnusx_data/_minio /data-root/onlyoffice-data /data-root/onlyoffice-logs && chown -R $(PUID):$(PGID) /data-root/logs/app /data-root/logs/celery /data-root/logs/snakemake /data-root/uploads /data-root/.tmp /data-root/.conda_envs && chown $(PUID):$(PGID) /data-root/users'

docker-fix-permissions: ## 手动修复 /data/cygnusx 权限事故（登录 500 / PermissionError 时使用）
	@echo "=> 修复 /data/cygnusx 非 root 服务写入权限..."
	@docker run --rm -v $(DATA_ROOT):/data-root python:3.11-slim sh -c 'mkdir -p /data-root/logs/app /data-root/logs/celery/tasks /data-root/logs/nginx /data-root/logs/snakemake /data-root/users /data-root/uploads /data-root/.tmp /data-root/.conda_envs /data-root/cygnusx_data/_pgdata /data-root/cygnusx_data/_redis && chown -R $(PUID):$(PGID) /data-root/logs/app /data-root/logs/celery /data-root/logs/snakemake /data-root/users /data-root/uploads /data-root/.tmp /data-root/.conda_envs'
	@echo "=> 权限修复完成。建议执行: docker restart cygnusx-web cygnusx-worker cygnusx-beat"

verify-persistent-data: ## 验证数据库挂载及用户、Agent、知识库记录数（恢复后可加 ARGS=--require-existing-data）
	./scripts/verify_persistent_data.sh $(ARGS)
...
wait-web: ## 等待 CygnusX Web API 健康（失败时输出后端日志）
	@echo "=> 等待 cygnusx-web 健康..."
	@for attempt in $$(seq 1 60); do \
		status=$$(docker inspect --format '{{.State.Status}}' cygnusx-web 2>/dev/null || true); \
		health=$$(docker inspect --format '{{if .State.Health}}{{.State.Health.Status}}{{end}}' cygnusx-web 2>/dev/null || true); \
		if [ "$$health" = "healthy" ]; then \
			echo "=> cygnusx-web 已健康。"; exit 0; \
		fi; \
		if [ "$$status" = "exited" ] || [ "$$status" = "dead" ]; then \
			echo "=> cygnusx-web 启动失败，最近日志：" >&2; \
			docker logs --tail 120 cygnusx-web >&2 || true; \
			exit 1; \
		fi; \
		sleep 2; \
	done; \
	echo "=> 等待 cygnusx-web 健康超时，最近日志：" >&2; \
	docker logs --tail 120 cygnusx-web >&2 || true; \
	exit 1
...
docker-build-sandboxes: ## 重建全部沙盒/分析运行时镜像（runtime 三件套 + Copilot 代码执行沙盒 + studio base/bio + 终端全家桶，任一失败即中止）
	...
	@echo "╚══════════════════════════════════════════════════════════════╝"
	@echo ""
	@echo "🧪 步骤 1/5：Studio 运行时三件套（cygnusx-analysis core/plot/scrna）..."
	deploy/runtime-images/build.sh
	@echo "   ✅ 步骤 1/5 完成"
	@echo ""
	@echo "📦 步骤 2/5：AI Copilot 代码执行沙盒镜像（cygnusx-sandbox-copilot:v0.0.2dev）..."
	docker build -t cygnusx-sandbox-copilot:v0.0.2dev -f deploy/sandbox/Dockerfile deploy/sandbox
	@echo "   ✅ 步骤 2/5 完成"
	@echo ""
	@echo "🛠️  步骤 3/5：Studio 沙盒三件套（cygnusx-sandbox base/bio/browser-office）..."
	deploy/studio/build.sh
	@echo "   ✅ 步骤 3/5 完成"
	@echo ""
	@echo "💻 步骤 4/5：终端基础镜像（cygnusx-sandbox-terminal:v0.0.2dev）..."
	...
	@i=0; for t in rnaseq scrna gatk ggplot2 assembly metagenomics; do \
		i=$$((i+1)); \
		echo ""; \
		echo "   ▶ [$$i/6] 构建 cygnusx-sandbox-$$t:v0.0.2dev ..."; \
		docker build -t cygnusx-sandbox-$$t:v0.0.2dev -f tool_configs/terminal/docker/Dockerfile.$$t tool_configs/terminal/docker/ || exit 1; \
		echo "   ✅ [$$i/6] cygnusx-sandbox-$$t:v0.0.2dev 完成"; \
	done
	@echo ""
	@echo "📋 镜像清单核对："
	@ok=0; miss=0; \
	for img in cygnusx-analysis:core-v0.0.2dev cygnusx-analysis:plot-v0.0.2dev cygnusx-analysis:scrna-v0.0.3dev \
		cygnusx-sandbox-copilot:v0.0.2dev cygnusx-sandbox-base:v0.0.2dev cygnusx-sandbox-bio:v0.0.2dev cygnusx-sandbox-browser-office:v0.0.2dev cygnusx-sandbox-terminal:v0.0.2dev \
		cygnusx-sandbox-rnaseq:v0.0.2dev cygnusx-sandbox-scrna:v0.0.2dev cygnusx-sandbox-gatk:v0.0.2dev \
		cygnusx-sandbox-ggplot2:v0.0.2dev cygnusx-sandbox-assembly:v0.0.2dev cygnusx-sandbox-metagenomics:v0.0.2dev; do \
	...
	echo ""; \
	echo "=> 共 14 个镜像：就位 $$ok 个，缺失 $$miss 个"; \
...
docker-build-worker: docker-build-enrichment docker-build-deg ## 构建富集/DEG 运行时与通用 Worker 镜像（系统发育 phylo-worker 已暂时下线）
	$(COMPOSE_WORKER) build worker

docker-up-worker: docker-network docker-build-enrichment docker-build-deg ## 构建并启动富集/DEG 运行时与通用 Worker 栈（phylo-worker 已暂时下线）
	...
	$(COMPOSE_WORKER) up -d --build worker

docker-up-cross-worker: docker-build-enrichment docker-build-deg ## 启动跨机器 Worker（要求 .env 已配置 CYGNUSX_CONTROL_HOST）
	@python3 scripts/render_worker_config.py
	@set -a; . data/.worker-config.env; test ! -f .env || . ./.env; set +a; \
		$(MAKE) --no-print-directory docker-runtime-dirs DATA_ROOT="$$WORKER_SHARED_DATA_DIR"
	CYGNUSX_WORKER_COMPOSE_OVERLAY=deploy/docker/docker-compose.cross-worker.yml \
		$(COMPOSE_WORKER) up -d --build worker
...
docker-stop-all: ## 停止全平台（主栈 + Worker + AgentTeams），可用 docker-reload 重新拉起
	...
	-@docker ps -a --format '{{.Names}}' | grep -E '^(cygnusx|omichub)-' | grep -v 'matrix-dev' | xargs -r docker rm -f 2>/dev/null
...
docker-purge: ## [危险] 彻底清理 Docker 资源：容器+数据卷+网络+本地镜像+前端构建产物（保留宿主机 /data/cygnusx 数据）
	...
	-docker rm -f cygnusx-nginx cygnusx-web cygnusx-beat cygnusx-flower cygnusx-db cygnusx-cache cygnusx-worker cygnusx-phylo-worker cygnusx-studio-egress-proxy 2>/dev/null
	-@docker ps -a --filter "name=studio-" -q | xargs -r docker rm -f 2>/dev/null
	@echo "=> 删除所有 CygnusX 数据卷（含 docker_ 前缀命名卷）..."
	-docker volume rm $$(docker volume ls -q --filter name=docker_) 2>/dev/null
	@echo "=> 删除 cygnusx 网络（含沙盒与 Studio 会话网络）..."
	...
	-docker rmi $$(docker images --filter reference=docker-web --filter reference=docker-beat --filter reference=docker-flower --filter reference=docker-studio-egress-proxy --filter reference=cygnusx-worker --filter reference=cygnusx-phylo-worker --filter reference=cygnusx-r-enrichment --filter reference=cygnusx-sandbox-terminal --filter reference=cygnusx-analysis -q) 2>/dev/null
	@echo "=> 清理前端构建产物..."
	rm -rf frontend/dist
	@echo "=> Docker 资源清理完成"
...
docker-reload: docker-network ## 重新构建前端并刷新 pgvector 主栈 + RocketMQ + AgentTeams Bridge + Worker (开发用)
	...
	@docker image inspect cygnusx-sandbox-terminal:v0.0.2dev >/dev/null 2>&1 && \
		echo "   ⏭️  cygnusx-sandbox-terminal:v0.0.2dev 已存在，跳过构建" || \
		docker build -t cygnusx-sandbox-terminal:v0.0.2dev tool_configs/terminal/docker/
	@echo ""
	@echo "🧪 步骤 3/11：构建 Studio 运行时镜像（core/plot/scrna 均已存在则跳过）..."
	@if docker image inspect cygnusx-analysis:core-v0.0.2dev >/dev/null 2>&1 && \
	    docker image inspect cygnusx-analysis:plot-v0.0.2dev >/dev/null 2>&1 && \
	    docker image inspect cygnusx-analysis:scrna-v0.0.3dev >/dev/null 2>&1; then \
		echo "   ⏭️  cygnusx-analysis 三个运行时镜像均已存在，跳过构建"; \
	else \
		deploy/runtime-images/build.sh; \
	fi
	@echo ""
	@echo "🖥️  步骤 4/12：构建 Studio browser-office 沙箱镜像（含 Chromium/Playwright/LibreOffice）..."
	@$(MAKE) --no-print-directory docker-build-studio-sandboxes
	@echo ""
	@echo "🧬 步骤 5/12：构建 GO / KEGG 富集 R 运行时镜像（已存在则跳过）..."
	@docker image inspect $(ENRICHMENT_DOCKER_IMAGE) >/dev/null 2>&1 && \
		echo "   ⏭️  $(ENRICHMENT_DOCKER_IMAGE) 已存在，跳过构建" || \
		$(MAKE) --no-print-directory docker-build-enrichment
	@echo ""
	@echo "📊 步骤 6/12：构建 DEG 差异表达分析 R 运行时镜像（已存在则跳过）..."
	@docker image inspect $(DEG_DOCKER_IMAGE) >/dev/null 2>&1 && \
		echo "   ⏭️  $(DEG_DOCKER_IMAGE) 已存在，跳过构建" || \
		$(MAKE) --no-print-directory docker-build-deg
	@echo ""
	@echo "🚀 步骤 7/12：拉取 RocketMQ NameServer/Broker 官方镜像..."
...
sync-knowledge: wait-web ## 同步通用、QC、Cloud 知识库及已登记 Wiki 正文到数据库
	...
	@docker exec cygnusx-web python scripts/import_qc_knowledge.py --auto-admin
	@echo ""
	@echo "☁️  同步 Cloud 知识库（递归 Markdown + PDF/DOCX/PPTX/图片附件索引）..."
	@docker exec cygnusx-web python scripts/import_cloud_knowledge.py --auto-admin
	@echo ""
...
check-alembic-heads: ## 检测 Alembic 迁移是否存在多分支 (Multiple Heads)
	...
	@sleep 3  # 等待 web 容器就绪
	@HEAD_COUNT=$$(docker exec cygnusx-web alembic heads 2>/dev/null | grep -c '(head)' || echo 0); \
	if [ "$$HEAD_COUNT" -gt 1 ]; then \
		...
		echo ""; \
		echo "  或手动分步："; \
		echo "    docker exec cygnusx-web alembic merge heads -m \"merge parallel branches\""; \
		echo "    docker exec cygnusx-web alembic upgrade head"; \
		echo "====================================================================="; \
		echo ""; \
	else \
		echo "=> ✅ 数据库版本树正常，未发现分叉。"; \
