...
| peak-annotation | ChIPseeker, HOMER, pyranges with nearest-TSS vs host-gene, ENCODE cCRE classification (PLS/pELS/dELS/CA-CTCF/CA-H3K4me3), GREAT, ChIP-Enrich |
...
| cut-and-run-tag | CUT&RUN (Skene Henikoff 2017) and CUT&Tag (Kaya-Okur 2019) with SEACR vs MACS2 peak calling (btaf375 2025 benchmark); E. coli spike-in carryover |
| spike-in-normalization | ChIP-Rx (Drosophila); RRPM vs Rx-Input; integration with DiffBind/DESeq2/edgeR/csaw; Patel et al 2024 failure modes; ChIPseqSpikeInFree post-hoc |
...
- "Annotate peaks to ENCODE cCREs (PLS / pELS / dELS / CA-CTCF)"
- "Call super-enhancers with ROSE2; spike-in normalize before cross-condition BET-inhibitor comparison"
- "Generate spike-in-scaled bigWigs with bamCoverage --scaleFactor alone (not combined with --normalizeUsing)"
...
conda install -c bioconda macs3 macs2 homer samtools bedtools idr phantompeakqualtools \
    deeptools meme bowtie2 cutadapt subread pygenometracks
...
# SpikeFlow (Snakemake wrapper): https://github.com/DavideBrex/SpikeFlow
...
# ROSE for super-enhancers (stjude/ROSE fork supports hg38; rose2's genomeDict does not)
...
- [Patel et al 2024 spike-in review](https://pmc.ncbi.nlm.nih.gov/articles/PMC12266361/)
