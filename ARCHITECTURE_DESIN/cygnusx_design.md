...
> **用途**：本文件是 CygnusX 新增、扩展或重构模块时的项目级设计基线。新增模块先按本文档完成设计，再开始编码；模块级设计文档可在此基础上补充领域细节。
>
> **最后更新**：2026-07-29  
...
Browser (Vue 3 + TypeScript + Naive UI)
  ...
  └─ frontend/src/composables/    可复用状态与副作用
             ...
             ▼
FastAPI (/api/v1)
  ├─ src/cygnusx/api/v1/          路由与依赖注入
  ├─ src/cygnusx/application/     DTO、应用服务、用例编排
  ├─ src/cygnusx/domain/          复杂业务规则与领域对象
  ├─ src/cygnusx/infrastructure/  数据库、缓存、外部服务、执行器
  └─ src/cygnusx/tools/           相对独立的生信工具模块
             │
             ▼
...
| 生信工具 | 独立工具页或工具后端能力 | `src/cygnusx/tools/<tool_key>/` | `views/BioTools/`、`components/bio-tools/` |
...
2. **工具箱展示**：工具箱按分组顺序和组内 `order` 升序展示，禁用工具不计数；未知分组归入“其他工具”。分组默认展开，折叠状态使用 `localStorage` 键 `cygnusX_tools_group_collapsed` 记忆，卡片必须继续复用 `components/bio-tools/ToolCard.vue`。
...
1. 在 `src/cygnusx/api/v1/` 新建模块路由，使用 `APIRouter()`；按资源命名，例如 `/api/v1/<resource>`。
2. 在 `src/cygnusx/api/v1/router.py` 统一挂载路由、前缀和 tags；不要在应用入口散落注册。
   - 例外：生信工具路由由 `register_tool_routers` 自动发现并挂载（`src/cygnusx/tools/__init__.py`，`router.py` 统一调用一次）；新增工具只需在 `src/cygnusx/tools/<工具名>/api.py` 声明 `router`/`prefix`/`tags`，无需改 `router.py`。
...
| 层 | 目录 | 职责 |
|---|---|---|
| DTO / Schema | `src/cygnusx/application/schemas/` | 请求校验、响应结构、跨 API 的稳定数据契约。 |
| 应用服务 | `src/cygnusx/application/services/` | 用例编排、事务边界、权限协作、调用仓储/外部服务。 |
| 领域层 | `src/cygnusx/domain/<module>/` | 多状态转换、复杂规则、值对象、实体与领域服务。 |
| 基础设施 | `src/cygnusx/infrastructure/` | ORM、Redis、文件存储、Docker、第三方 API、下载与执行适配器。 |
...
| 应用运行设置 | `src/cygnusx/core/config.py` + 环境变量 | 数据库、Redis、密钥、开关、外部端点等运行时差异。 |
...
- [ ] 后端 API 已挂载到 `src/cygnusx/api/v1/router.py`，请求/响应 DTO 完整且类型稳定。
...
- `docs/update_info/26.7.10/platform_workflow_monitor_template_architecture.md`：复杂跨层模块的完整设计样例。
- `docs/update_info/26.7.11/Kimi_Agent_系统发育树工具架构/`：工具模块前后端拆分与实现样例。
...
| 关注点 | 决策 | 文件 |
|---|---|---|
| 用户名列 | 后端 `GET /tasks` 列表通过 `task_service._attach_user_display_info` 按 `user_id` 回填 `username` 与 `nickname`（`TaskResponse` 已含两个字段），前端 `Task` 类型与 `TaskTable` 用户列渲染（`nickname \|\| username \|\| user_id`）均已具备，仅由 `showUser` prop 控制且默认关闭。**只在前端开启**，零后端改动。 | `views/TasksView.vue`（传 `show-user`） |
...
**契约对齐（§1.2）**：`TaskResponse.username` / `TaskResponse.nickname` 与前端 `Task.username?` / `Task.nickname?` 已存在并被列表接口回填，前端只做展示开关，不新增隐式字段。
...
文件名默认 `cygnusx-tasks-YYYYMMDD.xlsx`。导出按钮带 `loading` 与空数据禁用 + 文案提示。
