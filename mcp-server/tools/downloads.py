...
def register(mcp: FastMCP, api: OmicHubAPIClient) -> None:
    ...
    @mcp.tool()
    async def omichub_submit_download(
        source: str = "sra",
        accession: str = "",
        ...
        Args:
            source: 下载来源 - "sra"（公共数据库）/ "cloud_storage"（云存储）/ "direct_link"（直链）
            accession: 公共数据库登录号（PRJNA…/PRJEB…/SRR…/ERR…/DRR…/GSM…），source=sra 时必填
            download_method: SRA 下载方式 aws（默认，最快）/ aspera / ftp，仅 source=sra 有效
            multithreads: 并发下载 run 数，默认 4
            aws_threads: 单文件 AWS 并发线程数，默认 8
            cloud_provider: 云商 aliyun/volc/huawei，source=cloud_storage 时必填
            object_uri: 对象路径（oss:// / tos:// / obs:// 前缀），source=cloud_storage 时必填
            recursive: 云存储目录是否递归下载，默认 True
            links: HTTP/HTTPS/FTP 下载链接列表，source=direct_link 时必填
            target_directory: 下载到用户 raw_data 下的子目录（如 "project_x"），留空=raw_data 根目录
            dry_run: True 时仅模拟下载（仅 SRA），用于校验登录号
            user_confirmed: 设为 True 确认提交（提交任务前必须经用户确认）
        """
        ...
        try:
            ...
            return {
                ...
                "data": result,
                "next_steps": [
                    f"调用 omichub_get_download_progress(task_id='{task_id}') 查看逐 run 下载进度",
                    "也可用 omichub_list_downloads 查看全部下载任务状态",
                ],
            }
    ...
    @mcp.tool()
    async def omichub_list_downloads() -> dict:
    ...
    @mcp.tool()
    async def omichub_get_download_progress(task_id: str) -> dict:
